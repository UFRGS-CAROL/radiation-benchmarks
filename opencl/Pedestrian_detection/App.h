...
void App::run() {
	...
	ocl::HOGDescriptor gpu_hog(win_size, Size(16, 16), Size(8, 8), Size(8, 8),
			9, ocl::HOGDescriptor::DEFAULT_WIN_SIGMA, 0.2, gamma_corr,
			ocl::HOGDescriptor::DEFAULT_NLEVELS);
	HOGDescriptor cpu_hog(win_size, Size(16, 16), Size(8, 8), Size(8, 8), 9, 1,
		...
		while (running && !frame.empty()) {
			...
			if (use_gpu) {
				...
				if (!verify) {
					...
					cpu_hog.detectMultiScale(img, ref_rst, hit_threshold,
							win_stride, Size(0, 0), scale, gr_threshold - 2);
					double accuracy = checkRectSimilarity(img.size(), ref_rst,
							found);
			...
			if (output != "" && write_once) {
				...
				} else                    //write video
				{
					if (!video_writer.isOpened()) {
						video_writer.open(output, CV_FOURCC('x', 'v', 'i', 'd'),
								24, img_to_show.size(), true);
						if (!video_writer.isOpened())
...
double App::checkRectSimilarity(Size sz, std::vector<Rect>& ob1,
		std::vector<Rect>& ob2) {
	...
	if (sz1 != sz2) {
		...
		for (vector<Rect>::const_iterator r = ob1.begin(); r != ob1.end();
				r++) {
		...
		gpu_result.setTo(0);
		for (vector<Rect>::const_iterator r2 = ob2.begin(); r2 != ob2.end();
				r2++) {
