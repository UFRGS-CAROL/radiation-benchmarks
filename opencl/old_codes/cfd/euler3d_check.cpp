 ...
 */ 
#define GAMMA 1.4f
#define TEST_ITERATIONS 1 // number of times to be executed
#define KERNEL_ITERATIONS 1000 // number of times the kernel will compute for each TEST_ITERATION
#ifndef block_length
...
void check_results(cl_mem variables, int nel, int nelr){
	float* h_variables = new float[nelr*NVAR];
	download(h_variables, variables, nelr*NVAR);
	float gold;
	long int errors = 0;
	{
		std::ifstream file("density");
		file >> nel;
		file >> nelr;
		for(int i = 0; i < nel; i++){
			file >> gold;
			if(fabs((gold-h_variables[i + VAR_DENSITY*nelr])/gold) > 0.001)
				errors++;
		}
	}
...

	{
		...
		for(int i = 0; i < nel; i++)
		{
			for(int j = 0; j != NDIM; j++){
				file >> gold;
				if(fabs((gold-h_variables[i + (VAR_MOMENTUM+j)*nelr])/gold) > 0.001)
					errors++;
			}
		}
	}
	...
	{
		std::ifstream file("density_energy");
		file >> nel;
		file >> nelr;
		for(int i = 0; i < nel; i++){
			file >> gold;
			if(fabs((gold-h_variables[i + VAR_DENSITY_ENERGY*nelr])/gold) > 0.001)
				errors++;
		}
	}
	delete[] h_variables;
...
int main(int argc, char** argv){
  printf("WG size of kernel:initialize = %d, WG size of kernel:compute_step_factor = %d, WG size of kernel:compute_flux = %d, WG size of kernel:time_step = %d\n", BLOCK_SIZE_1, BLOCK_SIZE_2, BLOCK_SIZE_3, BLOCK_SIZE_4);
	...
	try{		
		...
		double kernel_time=0, check_time=0;
		for(int j = 0; j < TEST_ITERATIONS; j++){
			 std::cout << "Test number: " << j << std::endl;

		// Begin KERNEL_ITERATIONS
			time0 = get_time();
			...
			time1 = get_time();
			kernel_time = (double) (time1-time0) / 1000000;
			std::cout << "kernel time: " << kernel_time << std::endl;
			
			time0 = get_time();
			check_results(variables, nel, nelr);
			time1 = get_time();
			check_time = (double) (time1-time0) / 1000000;
			std::cout << "check time: " << check_time << std::endl << std::endl;
		}
/*
		_clFinish();
