...
void ocl_alloc_buffers(int mem_size){
	d_odata = clCreateBuffer(context, CL_MEM_READ_WRITE, mem_size, NULL, &err);
	CL_CHECK_ERROR(err);
	d_factor = clCreateBuffer(context, CL_MEM_READ_WRITE, mem_size/2, NULL, &err);
	CL_CHECK_ERROR(err);
	d_unit = clCreateBuffer(context, CL_MEM_READ_WRITE, sizeof(Cal_Unit), NULL, &err);
	CL_CHECK_ERROR(err);
}

void ocl_release_buffers(){
	clReleaseMemObject(d_odata);
	clReleaseMemObject(d_factor);
	clReleaseMemObject(d_unit);	
}

void ocl_set_kernel_args(){
	err = clSetKernelArg(fft_kernel, 0, sizeof(cl_mem), (void*)&d_unit);
	CL_CHECK_ERROR(err);
	err = clSetKernelArg(fft_kernel, 1, sizeof(cl_mem), (void*)&d_factor);
	CL_CHECK_ERROR(err);
	err = clSetKernelArg(fft_kernel, 2, sizeof(cl_mem), (void*)&d_odata);
	CL_CHECK_ERROR(err);	
}

void ocl_write_odata_buffer(Complex *odata, int mem_size){
	err = clEnqueueWriteBuffer(command_queue, d_odata, CL_TRUE, 0, mem_size, odata, 0, NULL, NULL);
	CL_CHECK_ERROR(err);	
}

void ocl_read_odata_buffer(Complex *odata, int mem_size){
	err = clEnqueueReadBuffer(command_queue, d_odata, CL_TRUE, 0, mem_size, odata, 0, NULL, NULL);
	CL_CHECK_ERROR(err);
}

void ocl_write_factor_buffer(Complex *h_factor, int mem_size){
	err = clEnqueueWriteBuffer(command_queue, d_factor, CL_TRUE, 0, mem_size, h_factor, 0, NULL, NULL);
	CL_CHECK_ERROR(err);	
}

void ocl_write_unit_buffer(Cal_Unit *h_unit, int mem_size){
	err = clEnqueueWriteBuffer(command_queue, d_unit, CL_TRUE, 0, mem_size, h_unit, 0, NULL, NULL);
	CL_CHECK_ERROR(err);
}

void ocl_exec_kernel(const long unsigned int global_wsize, const long unsigned int local_wsize){
	err = clEnqueueNDRangeKernel(command_queue, fft_kernel, 1, NULL, &global_wsize, &local_wsize, 0, NULL, NULL);
	CL_CHECK_ERROR(err);
...
void initOpenCL() {
    ...
    */
    ...
    FILE*  theFile = fopen("fft_kernel.cl", "r");
    if (!theFile) {
    ...
    CL_CHECK_ERROR(err);
    if (err != CL_SUCCESS) {
        printf("Error compiling fft kernel\n\n");
        char log[5000];
        size_t retsize = 0;
    ...
    fft_kernel = clCreateKernel(gpuprog, "fft_unit", &err);
    CL_CHECK_ERROR(err);
}
