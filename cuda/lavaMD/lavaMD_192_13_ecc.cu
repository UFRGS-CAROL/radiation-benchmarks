...
int main(int argc, char *argv []) {
	...
	char file_name[70];
	char file_name_log[70];
	char file_name_ecc[70];
	...
	strcat(file_name,"_LavaMD_192_13_ecc");
	strcpy(file_name_log, file_name);
	strcpy(file_name_ecc, file_name);

	strcat(file_name,".txt");
	strcat(file_name_log,"_log.txt");
	strcat(file_name_ecc,".ecc");
	...
	for(loop=0; loop<ITERACTIONS; loop++) {
		...
		if(loop%10 == 0){
			char cmd[150] = "nvidia-smi -i 0 -q |head -n 100 |tail -n 31 >> ";
			strcat(cmd, file_name_ecc);

			char cmd_date[100] = "date >> ";
			strcat(cmd_date, file_name_ecc);
		...
		if(part_error > 0 || (loop % 10 == 0)) {
			...
			char cmd_date[100] = "date >> ";
			char cmd_errors[100];
			sprintf(cmd_errors, "echo test number: %d, errors: %d, time: %f >> ", loop, part_error, total_kernel_time);
			 
			strcat(cmd_date, file_name_ecc);
			strcat(cmd_errors, file_name_ecc);
			system(cmd_date);
			system(cmd_errors);
			system(cmd);
