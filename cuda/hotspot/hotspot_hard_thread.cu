...
#define ITERACTIONS2 100		 //second loop
...
float time_elapsed) {
	...
	float last_result = 0.0;	 // To execute at least 2 times and compare if its the same result
	bool computed;
	int index;
	int error_detect = 0;
	for(int j = 0; j < 5; j++) {
...
if(j==0)
	temp_t[ty][tx]=1;
		...
		if (temp_t[ty][tx] == last_result)
			break;
		else{
			last_result = temp_t[ty][tx];
			if(j >= 2)
				error_detect = 1;
		}
	}
...
int main(int argc, char** argv) {
	...
	strcat(file_name,"_Hotspotx20thread");
	...
	for(loop=0; loop<ITERACTIONS; loop++) {
		...
		fprintf(log_file, "\nerrors detected: %d", errors_detected);
