...
void writeoutput(float *vect, int grid_rows, int grid_cols, char *file) {
	...
	if( (fp2 = fopen(OUTPUT_GOLD, "wb" )) == 0 )
		printf( "The file 'output_1024' was not opened\n" );

	for (i=0; i < grid_rows; i++)
	for (j=0; j < grid_cols; j++) {

		fwrite(&(vect[i*grid_cols+j]), 1, sizeof(float), fp2);
		index++;
	}

	printf( "The file 'output_1024' was written\n" );

	fclose(fp2);
}


void readinput(float *vect, int grid_rows, int grid_cols, char *file, char *file_out) {
...
int main(int argc, char** argv) {
	...
	readinput(FilesavingTemp, grid_rows, grid_cols, tfile, INPUT_TEMP);
	readinput(FilesavingPower, grid_rows, grid_cols, pfile, INPUT_POWER);
