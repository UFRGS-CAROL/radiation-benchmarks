...
int main(int argc, char *argv[])
{
	...
	if(argc != 4)
	{
		cout << "Usage: particles_gen [N_LARGE] [N_SMALL] [N_FIXED]" << endl;
		exit(1);
	}
	...
	else
	{
		...
		cout << "Generating inputs and gold for the following configuration: " << endl << "N_LARGE: " << numL << endl
		            << "N_SMALL: " << numS << endl << "N_FIXED: " << numF << endl << endl;
    	}
...
void Dem2Demo::step( float dt )
{
	...
	if(iter_count == FINAL_STATE_ITERATIONS - 1)
	{
		...
		gold.write((char*)m_posL, m_numLParticles*sizeof(float4));
		gold.write((char*)m_velL, m_numLParticles*sizeof(float4));
		gold.write((char*)m_forceL, m_numLParticles*sizeof(float4));

		gold.write((char*)m_posS, m_numSParticles*sizeof(float4));
		gold.write((char*)m_velS, m_numSParticles*sizeof(float4));
		gold.write((char*)m_forceS, m_numSParticles*sizeof(float4));
