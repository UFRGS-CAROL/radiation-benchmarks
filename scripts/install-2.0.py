...
def check():
    with open("/proc/cpuinfo") as datafile:
...
if check():
    ...
    varDir = input("Where is mounted the sd card??? ") + "/radiation-benchmarks";

logDir = varDir + "/log"
micLog = "/micNfs/carol/logs"

signalCmd = "killall -q -USR1 killtestSignal.py;"
signalCmd += " killall -q -USR1 test_killtest_commands_json.py; killall -q -USR1 python"
...
try:
    if not os.path.isdir(varDir):
        os.mkdir(varDir, 0o777)
    os.chmod(varDir, 0o777)
