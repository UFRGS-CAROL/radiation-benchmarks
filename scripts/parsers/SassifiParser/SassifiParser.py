...
def parse_csv(csv_input, logs_dir, cp):
    ...
    for i in range(0, INSTRUCTION_SIZE):
        writer.writerow({'instruction': ASID_STR[i], 'sdc_num': str(sdc_inst_count[i]), 'total_inst_count': str(total_inst_count[i]),
    ...
    for i in range(0, ERROR_MODEL_SIZE):
		writer.writerow({'instruction': EM_STR[i], 'sdc_num': str(sdc_em_count[i]), 'total_inst_count': str(total_em_count[i]),
    ...
    for kernel in inj_em_count_per_kernel:
        ...
        if kernel in sdc_em_count_per_kernel:
            ...
            for j in range(ERROR_MODEL_SIZE):
                writer.writerow({'instruction': EM_STR[j], 'sdc_num': str(err_list[j])})
    ...
    for kernel in inj_inst_count_per_kernel:
        ...
        if kernel in sdc_inst_count_per_kernel:
            ...
            for j in range(INSTRUCTION_SIZE):
                writer.writerow({'instruction': ASID_STR[j], 'sdc_num': str(err_list[j])})
