...
maxCMDexecution =  60*2# in seconds
...
def readCmdTimestamp( pos ):
	# Read the timestamp file
	try:
		fp = open(varDir+"command_execstart_"+str(pos),'r')
...
def selectCommand():
	...
	#	os.system("shutdown -r now")
	#	time.sleep(20)
	timestamp = readCmdTimestamp(i)
	now = int(time.time())
	if (now - timestamp) < timeWindow:
		return i#commandList[i][0]
...
try:
	...
	curCommandPos = selectCommand()
	curCommand = commandList[curCommandPos][0]
	...
	while True:
		...
		now = int(time.time())
		timestampCMDStart = readCmdTimestamp(curCommandPos)
		if (now - timestampCMDStart) > maxCMDexecution:
			for cmd in commandList:
				os.system(killcmd+" "+cmd[2])
			logMsg("Killing application excced maxCMDexecution time, current command: "+curCommand)
			curCommandPos = selectCommand()
			curCommand = commandList[curCommandPos][0]
