...
const FLOAT chip_height_hardened_1 = 0.016;
const FLOAT chip_height_hardened_2 = 0.016;
const FLOAT chip_width_hardened_1 = 0.016;
const FLOAT chip_width_hardened_2 = 0.016;
...
#ifdef OPEN
    omp_set_num_threads(READ_HARDENED_VAR(num_omp_threads_hardened_1, num_omp_threads_hardened_2, int, sizeof(int), "num_omp_threads"));
...
void compute_tran_temp(FLOAT *result, int num_iterations, FLOAT *temp, FLOAT *power, int row, int col)
{
    ...
    FLOAT grid_height = READ_HARDENED_VAR(chip_height_hardened_1, chip_height_hardened_2, FLOAT, sizeof(FLOAT), "chip_height") / row;
    FLOAT grid_width = READ_HARDENED_VAR(chip_width_hardened_1, chip_height_hardened_2, FLOAT, sizeof(FLOAT), "chip_width") / col;
...
void read_input(FLOAT *vect, int grid_rows, int grid_cols, char *file)
{
    ...
    FILE *fp;
    char str[STR_SIZE];
    FLOAT val_hardened_1;
    FLOAT val_hardened_2;
    ...
    for (i=0; i < grid_rows * grid_cols; i++) {
        ...
        if ((sscanf(str, "%f", &val_hardened_1) != 1) && (sscanf(str, "%f", &val_hardened_2) != 1) )
            fatal("invalid file format");
        vect[i] = READ_HARDENED_VAR(val_hardened_1, val_hardened_2, FLOAT, sizeof(FLOAT), "val");
    }
...
int main(int argc, char **argv)
{
    ...
    start_log_file((char *)"openMPHotspot_hardening_4", test_info);
