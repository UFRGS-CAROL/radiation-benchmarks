...
#include "./main.h"
#ifdef HARDENING
#include "./kernel/kernel_cpu_hardened.h"
#else
...
int main( int argc, char *argv [])
{
    ...
    dim_str dim_cpu;
    box_str* box_cpu;
    FOUR_VECTOR* rv_cpu_hardened_1;
    FOUR_VECTOR* rv_cpu_hardened_2;
    fp* qv_cpu;
    FOUR_VECTOR* fv_cpu;
    ...
    rv_cpu_hardened_1 = (FOUR_VECTOR*)malloc(dim_cpu.space_mem);
    rv_cpu_hardened_2 = (FOUR_VECTOR*)malloc(dim_cpu.space_mem);

    for(i=0; i<dim_cpu.space_elem; i=i+1) {
        fread(&(rv_cpu_hardened_1[i].v), 1, sizeof(double), file);
        fread(&(rv_cpu_hardened_1[i].x), 1, sizeof(double), file);
        fread(&(rv_cpu_hardened_1[i].y), 1, sizeof(double), file);
        fread(&(rv_cpu_hardened_1[i].z), 1, sizeof(double), file);
        rv_cpu_hardened_2[i].v = rv_cpu_hardened_1[i].v;
	rv_cpu_hardened_2[i].x = rv_cpu_hardened_1[i].x;
	rv_cpu_hardened_2[i].y = rv_cpu_hardened_1[i].y;
	rv_cpu_hardened_2[i].z = rv_cpu_hardened_1[i].z;
    }
...
#ifdef LOGS
    char test_info[100];
    snprintf(test_info, 100, "hardening_3 box:%d spaceElem:%ld cores:%d", dim_cpu.boxes1d_arg,dim_cpu.space_elem,dim_cpu.cores_arg);
    ...
    for(loop=0; loop<iteractions; loop++) {
        ...
        if(loop == 2) {
            ...
            rv_cpu_hardened_1[0].v = rv_cpu_hardened_1[0].v*2;
            rv_cpu_hardened_1[0].x = rv_cpu_hardened_1[0].x*-1;
            rv_cpu_hardened_1[0].y = rv_cpu_hardened_1[0].y*6;
            rv_cpu_hardened_1[0].z = rv_cpu_hardened_1[0].z*-3;
            rv_cpu_hardened_2[0].v = rv_cpu_hardened_2[0].v*2;
            rv_cpu_hardened_2[0].x = rv_cpu_hardened_2[0].x*-1;
            rv_cpu_hardened_2[0].y = rv_cpu_hardened_2[0].y*6;
            rv_cpu_hardened_2[0].z = rv_cpu_hardened_2[0].z*-3;
    ...
    free(rv_cpu_hardened_1);
    free(rv_cpu_hardened_2);
    free(qv_cpu);
    free(fv_cpu);
    free(box_cpu);
