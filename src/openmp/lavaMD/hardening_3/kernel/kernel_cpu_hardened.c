        ...
        for (k=0; k<(1+box[l].nn); k++) {
            ...
            for (i=0; i<NUMBER_PAR_PER_BOX; i=i+1) {
                ...
                for (j=0; j<NUMBER_PAR_PER_BOX; j=j+1) {
                    ...
                    // // coefficients
                    r2 = rA[i].v + READ_HARDENED_VAR_DOUBLE(rB_hardened_1[j].v, rB_hardened_2[j].v, "rv") - (rA[i].x * READ_HARDENED_VAR_DOUBLE(rB_hardened_1[j].x, rB_hardened_2[j].x, "rv") + rA[i].y * READ_HARDENED_VAR_DOUBLE(rB_hardened_1[j].y, rB_hardened_2[j].y, "rv") + rA[i].z * READ_HARDENED_VAR_DOUBLE(rB_hardened_1[j].z, rB_hardened_2[j].z, "rv"));
                    u2 = a2*r2;
                    vij= exp(-u2);
                    fs = 2.*vij;
                    d.x = rA[i].x - READ_HARDENED_VAR_DOUBLE(rB_hardened_1[j].x, rB_hardened_2[j].x, "rv");
                    d.y = rA[i].y - READ_HARDENED_VAR_DOUBLE(rB_hardened_1[j].y, rB_hardened_2[j].y, "rv");
                    d.z = rA[i].z - READ_HARDENED_VAR_DOUBLE(rB_hardened_1[j].z, rB_hardened_2[j].z, "rv");
                    fxij=fs*d.x;
                    fyij=fs*d.y;
                    fzij=fs*d.z;
