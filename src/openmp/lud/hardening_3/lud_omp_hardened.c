...
void lud_omp(float *a, int size)
{
    ...
    omp_set_num_threads(omp_num_threads);
		...
		int j_global_hardened_1, j_global_hardened_2;
                ...
                j_global_hardened_1 = offset + BS * (1 + chunk_idx%chunks_in_inter_row);
		j_global_hardened_2 = offset + BS * (1 + chunk_idx%chunks_in_inter_row);
                ...
                for (i = 0; i < BS; i++)
                {
                    ...
                    #pragma omp simd
                    for (j = 0; j < BS; j++) {
                        BB((i+i_global),(j+READ_HARDENED_VAR(j_global_hardened_1, j_global_hardened_2, int, sizeof(int), "j_global"))) -= sum[j];
                        sum[j] = 0.f;
                    }
                }
            }
