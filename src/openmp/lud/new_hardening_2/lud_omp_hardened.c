    ...
    omp_set_num_threads(omp_num_threads);
        ...
        for (offset = 0; offset < size - BS ; offset += BS)
        {
            ...
            for ( chunk_idx = 0; chunk_idx < chunks_in_inter_row; chunk_idx++)
            {
                ...
                float sum;
                float temp_hardened_1[BS*BS] __attribute__ ((aligned (64)));
		float temp_hardened_2[BS*BS] __attribute__ ((aligned (64)));

                for (i = 0; i < BS; i++) {
                    #pragma omp simd
                    for (j =0; j < BS; j++) {
                        temp_hardened_1[i*BS + j] = a[size*(i + offset) + offset + j ];
			temp_hardened_2[i*BS + j] = a[size*(i + offset) + offset + j ];
                    }
                }
                i_global = offset;
                j_global = offset;
                ...
                for (j = 0; j < BS; j++) {
                    for (i = 0; i < BS; i++) {
                        sum = 0.f;
                        for (k=0; k < i; k++) {
                            sum += READ_HARDENED_VAR_FLOAT(temp_hardened_1[BS*i +k], temp_hardened_2[BS*i +k], "temp") * BB((i_global+k),(j_global+j));
                        }
                        i_here = i_global + i;
                        j_here = j_global + j;
                ...
                for (i = 0; i < BS; i++) {
                    for (j = 0; j < BS; j++) {
                        sum = 0.f;
                        for (k=0; k < j; k++) {
                            sum += BB((i_global+i),(j_global+k)) * READ_HARDENED_VAR_FLOAT(temp_hardened_1[BS*k + j], temp_hardened_2[BS*k + j], "temp");
                        }
                        i_here = i_global + i;
                        j_here = j_global + j;
