...
#define LOOP_AND {\
        value_int = ref_int1; \
        asm volatile("and $0xFFFFFFFF, %0" : "+r" (value_int));\
        asm volatile("and $0xFFFFFFFF, %0" : "+r" (value_int));\
        asm volatile("and $0xFFFFFFFF, %0" : "+r" (value_int));\
        asm volatile("and $0xFFFFFFFF, %0" : "+r" (value_int));\
        asm volatile("and $0xFFFFFFFF, %0" : "+r" (value_int));\
        ...
        if (value_int != ref_int1) \
            snprintf(log[th_id][errors++], LOG_SIZE, "IT:%"PRIu64" POS:%d TH:%d OP:AND REF:0x%08x WAS:0x%08x", i, j, th_id, ref_int1, value_int); \
                }
...
int main (int argc, char *argv[]) {
    ...
    for (i = 0; i < repetitions; i++) {
        ...
        //======================================================================P
        // Parallel region
        #pragma offload target(mic:0) inout(log)
        {
