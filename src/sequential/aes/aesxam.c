...
{   char            inbuf[16], outbuf[16]={0x60,0x53,0x7f,0x1c,0xd4,0x5c,0x92,0x66,0x26,0x11,0xc1,0x8e,0x5f,0xd9,0xb8,0x69};
...
int main(int argc, char *argv[])
{
    ...
    //int diff;
    //printf("comecou\n");
    if(argc != 8 || (toupper(*argv[6]) != 'D' && toupper(*argv[6]) != 'E'))
    {
        printf("usage: rijndael IP port in_filename out_filename [d/e] key_in_hex\n");
        exit(-1);
    }
    unsigned int port = atoi(argv[2]);
    setup_socket(argv[1],port);

    if(!(fout = fopen(argv[4], "wb")))  /* try to open the output file */
    {
    ...
    while(1){
        ...
        if(!(fin = fopen(argv[3], "rb")))   /* try to open the input file */
        {
        ...
        {                           /* encryption in Cipher Block Chaining mode */
            ...
            err = encfile(fin, fout, ctx, argv[3]);
        }
        else
        {                           /* decryption in Cipher Block Chaining mode */
            ...
            err = decfile(fin, fout, ctx, argv[3], argv[4]);
