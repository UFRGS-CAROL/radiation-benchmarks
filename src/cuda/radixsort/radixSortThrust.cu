...
void getParams(int argc, char *argv[], parameters_t *params)
{
	...
	if (checkCmdLineFlag(argc, (const char **)argv, "keysonly")) {
		params->keysOnly = 1;
	}
	...
	if (checkCmdLineFlag(argc, (const char **)argv, "gold")) {
		...
		snprintf(params->goldName, 100, "radixsort_Gold%s%i", (params->keysOnly ? "Keys" : "KeysVals"), (signed int)params->numElements);
...
int compareGoldOutput(parameters_t *params, uint *h_keysOut, uint *h_valuesOut, uint *h_keysGold, uint *h_valuesGold)
{
	...
	#pragma omp parallel num_threads(2) shared(flag)
	{
		...
		} else if (!(params->keysOnly)) { // Thread 1
			register unsigned int i;
			#pragma omp parallel for
			for (i=0; i<params->numElements; i++)
			{
...
void testSort(parameters_t *params)
{
    ...
    if (params->verbose)
        printf("\nSorting %d %d-bit int keys %s\n\n", params->numElements, keybits, params->keysOnly ? "(only)" : "and values");
    ...
    if (cudaSuccess == cudaGetDevice(&deviceID))
    {
        ...
        unsigned int totalMem = (params->keysOnly ? 2 : 4) * params->numElements * sizeof(uint);
    ...
    for (unsigned int loop1 = 0; loop1 < params->numIterations; loop1++)
    {
		...
		*/
        ...
        if (params->keysOnly)
            thrust::sort(thrust::device_ptr<uint> (d_keys), thrust::device_ptr<uint> (d_keys+params->numElements));
        else
...
int main(int argc, char **argv)
{
	...
	#ifdef LOGS
		char test_info[90];
		snprintf(test_info, 90, "size:%d keysOnly:%d", params->numElements, params->keysOnly);
