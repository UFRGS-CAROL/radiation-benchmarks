...
//================== CUDA error handlers
	...
	const char *erro;
//====================================
	malloc = cudaMalloc( ( void** ) &d_INPUT, matrixSize * sizeof( float ) );
	...
	malloc = cudaMalloc( ( void** ) &d_OUTPUT, matrixSize * sizeof( float ) );
	erro = cudaGetErrorString(malloc);
	if(strcmp(erro, "no error") != 0) {
#ifdef LOGS
...
//================== CUDA error handlers
	...
	const char *erro;
//====================================
	mcpy = cudaMemset(d_OUTPUT, 0, matrixSize * sizeof (float));
	...
	mcpy = cudaMemcpy( d_INPUT, INPUT, matrixSize * sizeof( float ), cudaMemcpyHostToDevice ); // PUSH A
	erro = cudaGetErrorString(mcpy);
	if(strcmp(erro, "no error") != 0) {
#ifdef LOGS
...
void generateInputMatrix(float *m) {
	if (!(f_INPUT = fopen(input_matrix_path, "wb"))) {
		...
		for (int i = 0; i < DEFAULT_INPUT_SIZE; i++) {
			float tempArray[DEFAULT_INPUT_SIZE];
			#pragma omp parallel for
			for (int j = 0; j < DEFAULT_INPUT_SIZE; j++)
				tempArray[j] = (float) rand() / 32768.0;
			size_t ret_value = 0;
			ret_value = fwrite(tempArray, DEFAULT_INPUT_SIZE * sizeof(float), 1, f_INPUT);
			if (ret_value != 1) {
...
void writeGoldToFile(float *m) {
	if (!(f_GOLD = fopen(gold_matrix_path, "wb"))) {
		...
		size_t ret_value = 0;
		for (int i = 0; i < k; i++) {
			ret_value = fwrite(&(m[i * k]), k * sizeof(float), 1, f_GOLD);
			if (ret_value != 1) {
...
//================== Read inputs to HOST memory
	...
	if (f_INPUT) {
		...
		for(i=0; i<k; i++)
		{
			ret_value = fread (&(INPUT[ k * i ]), sizeof(float)*k, 1, f_INPUT);
			if (ret_value != 1) {
	...
	if (!generate) {
		...
		for(i=0; i<k; i++)
		{
			ret_value = fread (&(GOLD[ k * i ]), sizeof(float)*k, 1, f_GOLD);
			if (ret_value != 1) {
...
void usage() {
    printf("Usage: slud -size=N [-generate] [-input=<path>] [-gold=<path>] [-iterations=N] [-verbose] [-no-warmup]\n");
}
...
//================== Read test parameters
    ...
    else
    {
        ...
        snprintf(input_matrix_path, 100, "slud_input_%i.matrix", (signed int)DEFAULT_INPUT_SIZE);
...
//================== Init logs
	...
	if (!generate) start_log_file("cudaSLUD", test_info);
#endif
//====================================

//================== Alloc HOST memory
	INPUT = ( float* ) malloc( matrixSize * sizeof( float ) );
...
//====================================
	...
	{//================== Global test loop
        ...
        //if (kernel_errors != 0) {
        checkCudaErrors( cudaMemcpy(INPUT, d_OUTPUT, matrixSize * sizeof( float ), cudaMemcpyDeviceToHost) );
		if (generate) {
