...
int k=0;
int ensure_gold=5; // number of times it will re-execute the gold so to check it is ok
int ensure_gold_failsafe=20; // maximum number of times it will retry re-executing the gold generation
int size;
double *A, *B, *C, *GOLD;
...
void usage() {
    printf("Usage: generateMatricesDouble -size=N [-generator_debug] [-host_check] [-input_a=<path>] [-input_b=<path>] [-gold=<path>] [-ensure_gold=N]\n");
}
...
bool badass_memcmp(double *gold, double *found, unsigned long n) {
	...
	for (int i=0; i < n; i++) {
		if (gold[i] != found[i]) {
			if (flag == false) printf("memcmp found an error at position [%d]: gold: %1.20e | output:%1.20e\n", i, gold[i], found[i]);
			flag = true;
			count++;
		}
	}
	if (flag) printf(">> Total number of errors: %d\n", count);
	return flag;
}
...
void generateGoldMatrixHalf()
{
	...
	int gold_retry_count = 0;
	int gold_safe_count = 0;
	...
	do {
		...
		printf("cudaMxM... k=%d (Safe %d in %d attemps (retry count: %d))\n", k, gold_safe_count + 1, ensure_gold, gold_retry_count);
		double time = mysecond();
		...
		checkCudaErrors( cudaMemcpy(C, d_C, size * sizeof( double ), cudaMemcpyDeviceToHost) );

		if (gold_safe_count == 0) {
			// Gold is empty. Copy to gold directly
			memcpy(GOLD, C, size * sizeof( double ));
			gold_safe_count = 1;
		} else {
			if (badass_memcmp(GOLD, C, size)) {
				gold_safe_count = 1;
				memcpy(GOLD, C, size * sizeof( double )); // This is the newest correct gold
			} else {
				gold_safe_count++;
			}
		}
	} while((gold_safe_count < ensure_gold) && (ensure_gold_failsafe > gold_retry_count));

	cudaFree( d_A );
	cudaFree( d_B );
	cudaFree( d_C );

	if (gold_safe_count < ensure_gold) {
		printf("Fatal!! Retry limit reached. Bailing...\n");
		exit(EXIT_FAILURE);
	}
...
//================== Read parameters
    ...
    else
    {
        ...
        snprintf(a_matrix_path, 100, "dmxm_a_%i.matrix", (signed int)DEFAULT_INPUT_SIZE);
    ...
    else
    {
        ...
        snprintf(b_matrix_path, 100, "dmxm_b_%i.matrix", (signed int)DEFAULT_INPUT_SIZE);
        printf("Using default input_a path: %s\n", b_matrix_path);
    }
	...
	if (checkCmdLineFlag(argc, (const char **)argv, "ensure_gold"))
    {
        ensure_gold = getCmdLineArgumentInt(argc, (const char **)argv, "size");

        if ((ensure_gold <= 0))
        {
            printf("Invalid input given on the command-line: %d\n", k);
            printf("exit on line: %d", __LINE__); exit(EXIT_FAILURE);
        }
    }
//====================================
