...
void activate_array_ongpu(float *x, int n, ACTIVATION a) {
	activate_array_kernel<<<cuda_gridsize(n), BLOCK_SIZE>>>(x, n, a);
	CudaCheckError();

}

void gradient_array_ongpu(float *x, int n, ACTIVATION a,
		float *delta) {
...
void ActivationLayer::back_prop(){
	...
	float *x = this->output_.d_data();
	float *delta = this->g_.d_data();


//    gradient_array_ongpu(x, n, RELU, delta);
//    copy_ongpu(n, l.delta_gpu, 1, net.delta_gpu, 1);
