...
__global__ void forward_parallel(__float* input_buf,
__float* weight_buf,
__float* b_buf,
__float* output_buf,
int in_width,
...
__global__ void forward_batch_more(float* input_batch_buf, float* weight_buf,
	...
	if (get_global_id(0) > out_depth * out_width)
		return;
	if (get_global_id(1)
			> ((batch_size + THREAD_TASKS - 1) / THREAD_TASKS * out_height))
		return;
		...
		for (unsigned int task = task_start; task < task_end; task++) {
			for (unsigned int y = 0; y < kernel_size; y++) {
				for (unsigned int x = 0; x < kernel_size; x++) {
					...
					// compute the convolution
					sum[task % THREAD_TASKS] += input_buf_ele
	...
	for (unsigned int task = task_start; task < task_end; task++) {
		...
		output_batch_buf[out_index] = sigmod(
				sum[task % THREAD_TASKS] + b_buf[b_index]);
	}
}
