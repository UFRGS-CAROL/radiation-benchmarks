...
deconvolutional_layer parse_deconvolutional(list *options, size_params params) {
	...
	deconvolutional_layer layer = make_deconvolutional_layer(batch, h, w, c, n,
...
convolutional_layer parse_convolutional(list *options, size_params params) {
	...
	convolutional_layer layer = make_convolutional_layer(batch, h, w, c, n,
			size, stride, padding, activation, batch_normalize, binary, xnor);
...
layer parse_gru(list *options, size_params params) {
	...
	layer l = make_gru_layer(params.batch, params.inputs, output,
			params.time_steps, batch_normalize);
...
softmax_layer parse_softmax(list *options, size_params params) {
	...
	softmax_layer layer = make_softmax_layer(params.batch, params.inputs,
			groups);
...
int is_maxpool(section *s) {
	return (strcmp(s->type, "[max]") == 0 || strcmp(s->type, "[maxpool]") == 0);
}
int is_avgpool(section *s) {
	return (strcmp(s->type, "[avg]") == 0 || strcmp(s->type, "[avgpool]") == 0);
}
int is_dropout(section *s) {
...
int is_softmax(section *s) {
	return (strcmp(s->type, "[soft]") == 0 || strcmp(s->type, "[softmax]") == 0);
}
int is_route(section *s) {
	...
	for (i = 0; i < net.n; ++i) {
		...
		if (l.type == CONVOLUTIONAL) {
			...
			for (j = 0; j < l.n; ++j) {
				...
				fwrite(l.weights + index, sizeof(float), l.c * l.size * l.size,
						fp);
				for (k = 0; k < l.c * l.size * l.size; ++k)
