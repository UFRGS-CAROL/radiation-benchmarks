...
void backward_normalization_layer(const layer layer, network_state state) {
	...
	int h = layer.h;
	int c = layer.c;
	pow_cpu(w * h * c * layer.batch, -layer.beta, layer.norms, 1, state.delta,
			1);
...
void forward_normalization_layer_gpu(const layer layer, network_state state)
{
	...
	int h = layer.h;
	int c = layer.c;
	scal_ongpu(w*h*c*layer.batch, 0, layer.squared_gpu, 1, state.st_handle.stream);

	for(b = 0; b < layer.batch; ++b) {
		float *squared = layer.squared_gpu + w*h*c*b;
		float *norms = layer.norms_gpu + w*h*c*b;
		float *input = state.input + w*h*c*b;
		pow_ongpu(w*h*c, 2, input, 1, squared, 1, state.st_handle.stream);

		const_ongpu(w*h, layer.kappa, norms, 1, state.st_handle.stream);
		for(k = 0; k < layer.size/2; ++k) {
			axpy_ongpu(w*h, layer.alpha, squared + w*h*k, 1, norms, 1, state.st_handle.stream);
		}

		for(k = 1; k < layer.c; ++k) {
			copy_ongpu(w*h, norms + w*h*(k-1), 1, norms + w*h*k, 1, state.st_handle.stream);
			int prev = k - ((layer.size-1)/2) - 1;
			int next = k + (layer.size/2);
			if(prev >= 0) axpy_ongpu(w*h, -layer.alpha, squared + w*h*prev, 1, norms + w*h*k, 1, state.st_handle.stream);
			if(next < layer.c) axpy_ongpu(w*h, layer.alpha, squared + w*h*next, 1, norms + w*h*k, 1, state.st_handle.stream);
		}
	}
	pow_ongpu(w*h*c*layer.batch, -layer.beta, layer.norms_gpu, 1, layer.output_gpu, 1, state.st_handle.stream);
	mul_ongpu(w*h*c*layer.batch, state.input, 1, layer.output_gpu, 1, state.st_handle.stream);
}
...
void backward_normalization_layer_gpu(const layer layer, network_state state)
{
	...
	int h = layer.h;
	int c = layer.c;
	pow_ongpu(w*h*c*layer.batch, -layer.beta, layer.norms_gpu, 1, state.delta, 1, state.st_handle.stream);
	mul_ongpu(w*h*c*layer.batch, layer.delta_gpu, 1, state.delta, 1, state.st_handle.stream);
}
#endif
