...
region_layer make_region_layer(int batch, int w, int h, int n, int classes,
		int coords) {
	region_layer l = { 0 };
	l.type = REGION;
...
void forward_region_layer(const region_layer l, network_state state) {
	...
	for (b = 0; b < l.batch; ++b) {
		for (i = 0; i < l.h * l.w * l.n; ++i) {
			...
			if (l.softmax) {
				softmax_array(l.output + index + 5, l.classes, 1,
						l.output + index + 5);
			}
		}
	}
	...
	*(l.cost) = 0;
	for (b = 0; b < l.batch; ++b) {
		for (j = 0; j < l.h; ++j) {
			for (i = 0; i < l.w; ++i) {
				for (n = 0; n < l.n; ++n) {
					int index = size * (j * l.w * l.n + i * l.n + n)
							+ b * l.outputs;
					box pred = get_region_box(l.output, l.biases, n, index, i,
							j, l.w, l.h);
					float best_iou = 0;
					for (t = 0; t < 30; ++t) {
						box truth = float_to_box(
								state.truth + t * 5 + b * l.truths);
						if (!truth.x)
							break;
						float iou = box_iou(pred, truth);
						if (iou > best_iou)
					...
					if (*(state.net.seen) < 6400) {
						...
						truth.w = .5;
						truth.h = .5;
						delta_region_box(truth, l.output, l.biases, n, index, i,
								j, l.w, l.h, l.delta, .01);
		...
		for (t = 0; t < 30; ++t) {
			...
			for (n = 0; n < l.n; ++n) {
				int index = size * (j * l.w * l.n + i * l.n + n)
						+ b * l.outputs;
				box pred = get_region_box(l.output, l.biases, n, index, i, j,
						l.w, l.h);
				printf("pred: (%f, %f) %f x %f\n", pred.x * l.w - i - .5,
						pred.y * l.h - j - .5, pred.w, pred.h);
				pred.x = 0;
				pred.y = 0;
			...
			printf("%d %f (%f, %f) %f x %f\n", best_n, best_iou,
					truth.x * l.w - i - .5, truth.y * l.h - j - .5, truth.w,
					truth.h);
			...
			l.delta[best_index + 4] = l.object_scale
					* (1 - l.output[best_index + 4])
					* logistic_gradient(l.output[best_index + 4]);
			if (l.rescore) {
				l.delta[best_index + 4] = l.object_scale
						* (iou - l.output[best_index + 4])
						* logistic_gradient(l.output[best_index + 4]);
			}
			...
			for (n = 0; n < l.classes; ++n) {
				l.delta[best_index + 5 + n] =
						l.class_scale
								* (((n == class) ? 1 : 0)
										- l.output[best_index + 5 + n]);
				if (n == class)
					avg_cat += l.output[best_index + 5 + n];
			}
			/*
			 if(0){
	...
	*(l.cost) = pow(mag_array(l.delta, l.outputs * l.batch), 2);
	printf(
			...
			avg_iou / count, avg_cat / count, avg_obj / count,
