...
static void args_init_and_setnull(Args *arg) {
 ...
 */
static int check_args(const Args arg) {
    //check config_file
 ...
 */
static void print_args(const Args arg) {
    printf(
            "execution type = %s\n"
                    "execution model = %s\n"
                    "config file = %s\n"
 ...
 * return 0 ok, -1 wrong
 */
static int parse_arguments(Args *to_parse, int argc, char **argv) {
...
static void usage(char **argv, char *model, char *message) {
