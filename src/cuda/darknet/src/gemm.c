...
/**
 *
 * the matrices in this function must be allocated with n+1 lines and m+1 collums
 **/
int gemm_ongpu_abft(int TA, int TB, int M, int N, int K, float ALPHA,
		float *A_gpu, int lda,
		float *B_gpu, int ldb,
		float BETA,
		float *C_gpu, int ldc) {
	...
	check_error(status);
	//gemm(a, b, c, lin_a + 1, col_a, lin_b, col_b+1);
	...
	for (j = 0; j < col_a; j++) {
		...
		if (c[lin_a][j] != acc) {
			printf("lin - position corrupted [%ld][%ld] - exp chsum %lf got chsum %lf diff - %lf\n",lin_a, j, c[lin_a][j], acc, c[lin_a][j] - acc);
			ret++;
		}
	}
	...
	for (i = 0; i < lin_b; i++) {
		...
		if (fabs(c[i][col_b] - acc) >= MAX_THRESHOLD) {
			printf("collum - position corrupted [%ld][%ld] - exp chsum %lf got chsum %lf diff %lf\n",i, col_b, c[i][col_b], acc, c[i][col_b] - acc);
			ret++;
		}
	}
	return ret;
