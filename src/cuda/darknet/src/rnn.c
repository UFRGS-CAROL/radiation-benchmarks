...
void reset_rnn_state(network net, int b, cudaStream_t stream) {
	int i;
	for (i = 0; i < net.n; ++i) {
#ifdef GPU
		layer l = net.layers[i];
		if(l.state_gpu) {
			fill_ongpu(l.outputs, 0, l.state_gpu + l.outputs*b, 1, stream);
		}
#endif
	}
}
...
void valid_char_rnn(char *cfgfile, char *weightfile, char *seed) {
	...
	while (c != EOF) {
		...
		printf("%d Perplexity: %4.4f    Word Perplexity: %4.4f\n", count,
				pow(2, -sum / count), pow(2, -sum / words));
	}
}

void vec_char_rnn(char *cfgfile, char *weightfile, char *seed, cudaStream_t st) {
	char *base = basecfg(cfgfile);
	fprintf(stderr, "%s\n", base);
...
void run_char_rnn(int argc, char **argv) {
	...
	if (0 == strcmp(argv[2], "train")) {
		train_char_rnn(cfg, weights, filename, clear, tokenized, stream);
		...
		vec_char_rnn(cfg, weights, seed, stream);
