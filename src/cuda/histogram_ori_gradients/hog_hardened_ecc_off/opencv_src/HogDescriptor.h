...
class HogDescriptor {
protected:
	int computeBlockHistograms(const cv::gpu::GpuMat& img);
...
public:
	...
	int detect(const cv::gpu::GpuMat& img, vector<cv::Point>& found_locations,
			double hit_threshold = 0, cv::Size win_stride = cv::Size(), cv::Size padding =
					cv::Size());

	int detectMultiScale(const cv::gpu::GpuMat& img, vector<cv::Rect>& found_locations,
			double hit_threshold = 0, cv::Size win_stride = cv::Size(), cv::Size padding =
