...
HOGDescriptor::HOGDescriptor(Size win_size, Size block_size, Size block_stride, Size cell_size,
...
GpuMat HOGDescriptor::getBuffer(const Size& sz, int type, GpuMat& buf)
{
...
GpuMat HOGDescriptor::getBuffer(int rows, int cols, int type, GpuMat& buf)
{
    return getBuffer(Size(cols, rows), type, buf);
}


void HOGDescriptor::computeGradient(const GpuMat& img, GpuMat& _grad, GpuMat& _qangle)
{
    ...
    {
        case CV_8UC1:
            compute_gradients_8UC1(nbins, img.rows, img.cols, img, angleScale, _grad, _qangle, gamma_correction);
            break;
        case CV_8UC4:
            compute_gradients_8UC4(nbins, img.rows, img.cols, img, angleScale, _grad, _qangle, gamma_correction);
            break;
    }
}


void HOGDescriptor::computeBlockHistograms(const GpuMat& img)
{
    cv::Size blocks_per_win = numPartsWithin(win_size, block_size, block_stride);
    set_up_constants(nbins, block_stride.width, block_stride.height, blocks_per_win.width, blocks_per_win.height);
    ...
    compute_hists(nbins, block_stride.width, block_stride.height, img.rows, img.cols,
                        grad, qangle, (float)getWinSigma(), block_hists.ptr<float>());

    normalize_hists(nbins, block_stride.width, block_stride.height, img.rows, img.cols,
                         block_hists.ptr<float>(), (float)threshold_L2hys);
}


void HOGDescriptor::getDescriptors(const GpuMat& img, Size win_stride, GpuMat& descriptors, int descr_format)
{
    ...
    case DESCR_FORMAT_ROW_BY_ROW:
        extract_descrs_by_rows(win_size.height, win_size.width, block_stride.height, block_stride.width,
                                    win_stride.height, win_stride.width, img.rows, img.cols, block_hists.ptr<float>(), descriptors);
        break;
    case DESCR_FORMAT_COL_BY_COL:
        extract_descrs_by_cols(win_size.height, win_size.width, block_stride.height, block_stride.width,
                                    win_stride.height, win_stride.width, img.rows, img.cols, block_hists.ptr<float>(), descriptors);
        break;
    default:
...
void HOGDescriptor::computeConfidence(const GpuMat& img, vector<Point>& hits, double hit_threshold,
                          Size win_stride, Size padding, vector<Point>& locations, vector<double>& confidences)
{
  ...
  compute_confidence_hists(win_size.height, win_size.width, block_stride.height, block_stride.width,
...
void HOGDescriptor::computeConfidenceMultiScale(const GpuMat& img, vector<Rect>& found_locations,
                            ...
                            vector<HOGConfidence> &conf_out, int group_threshold)
{
    ...
    for (size_t i = 0; i < level_scale.size(); i++)
    {
        ...
        else
        {
            ...
            switch (img.type())
            {
            case CV_8UC1: resize_8UC1(img, smaller_img); break;
            case CV_8UC4: resize_8UC4(img, smaller_img); break;
            }
        }
...
void HOGDescriptor::detect(const GpuMat& img, vector<Point>& hits, double hit_threshold, Size win_stride, Size padding)
{
    CV_Assert(img.type() == CV_8UC1 || img.type() == CV_8UC4);
    CV_Assert(padding == Size(0, 0));
    ...
    classify_hists(win_size.height, win_size.width, block_stride.height, block_stride.width,
                        win_stride.height, win_stride.width, img.rows, img.cols, block_hists.ptr<float>(),
...
void HOGDescriptor::detectMultiScale(const GpuMat& img, vector<Rect>& found_locations, double hit_threshold,
                                              Size win_stride, Size padding, double scale0, int group_threshold)
{
    ...
    for (size_t i = 0; i < level_scale.size(); i++)
    {
        ...
        else
        {
            ...
            switch (img.type())
            {
                case CV_8UC1: resize_8UC1(img, image_scales[i]); break;
                case CV_8UC4: resize_8UC4(img, image_scales[i]); break;
            }
...
cv::Size HOGDescriptor::numPartsWithin(cv::Size size, cv::Size part_size, cv::Size stride)
{
