...
void read_input(XYZ *in, const Params &p, int in_size) {
	...
	int k = 0, ic = 0;
	XYZ *v = (XYZ*) malloc(sizeof(XYZ) * in_size);
#if DOUBLE_PRECISION
...
int main(int argc, char **argv) {
	...
	XYZ *h_in = (XYZ *) malloc(sizeof(XYZ) * in_size);
	XYZ *gold = (XYZ *) malloc(sizeof(XYZ) * out_size);
