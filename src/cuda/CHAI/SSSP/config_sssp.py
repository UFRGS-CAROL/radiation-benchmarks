...
def untar_graphs(file_path):
    ...
    while not os.path.isfile(file_path + "/" + INPUT):
        print "tar xzf " + file_path + "/" + INPUT + ".tar.gz -C " + file_path + "/"
        if os.system("tar xzf " + file_path + "/" + INPUT + ".tar.gz -C " + file_path + "/") != 0:
            print "Something went wrong with untar of " + file_path + "/" + INPUT + " file. Trying again"
...
def main(board):
    print "Generating SSSP for CUDA on " + str(board)
    ...
    src_bfs = install_dir + "src/cuda/CHAI/SSSP"
    ...
    for j in THREADS_HOST:
        ...
        output_file = data_path + "/graph_" + INPUT + "_proportion_" + str(j) + ".gold"
