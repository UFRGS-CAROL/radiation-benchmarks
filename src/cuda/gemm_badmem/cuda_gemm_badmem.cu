...
#undef SAFE_MALLOC // TODO: FIX THIS
...
void freeCudaMemory() {
	...
	if (deviceMemory == NULL) 
		return;
	checkFrameworkErrors( cudaFree(deviceMemory) );
	deviceMemory = NULL;
}
...
__global__ void MemoryTester(BYTE* first, BYTE* second, int n) {
	...
	int threadOffset = ty * n + tx;
	if (threadOffset > n*n)
		return;
	if (first[threadOffset] != second[threadOffset]) 
		atomicAdd(&memoryFailures, 1);
}
...
bool testOffsetWithValues(size_t offset, size_t size) {
	...
	dim3 blockSize(32, 32);
	dim3 gridSize(ceil(n / 32), ceil(n / 32));
	checkFrameworkErrors( cudaMemset(deviceMemory + offset, 0x55, size) );
	checkFrameworkErrors( cudaMemset(deviceMemory + offset + size, 0x55, size) );
	checkFrameworkErrors( cudaMemcpyToSymbol(memoryFailures, &hostMemoryFailures, sizeof(unsigned long long int)) );
	checkFrameworkErrors( cudaDeviceSynchronize() );
	MemoryTester<<<gridSize, blockSize>>>((BYTE*) deviceMemory + offset, (BYTE*) deviceMemory + offset + size, n);
	checkFrameworkErrors( cudaPeekAtLastError() );
	checkFrameworkErrors( cudaDeviceSynchronize() );
	checkFrameworkErrors( cudaMemcpyFromSymbol(&hostMemoryFailures, memoryFailures, sizeof(unsigned long long int)) );
	if (hostMemoryFailures != 0)
		return false;

	checkFrameworkErrors( cudaMemset(deviceMemory + offset, 0xAA, size) );
	checkFrameworkErrors( cudaMemset(deviceMemory + offset + size, 0xAA, size) );
	checkFrameworkErrors( cudaMemcpyToSymbol(memoryFailures, &hostMemoryFailures, sizeof(unsigned long long int)) );
	checkFrameworkErrors( cudaDeviceSynchronize() );
	MemoryTester<<<gridSize, blockSize>>>((BYTE*) deviceMemory + offset, (BYTE*) deviceMemory + offset + size, n);
	checkFrameworkErrors( cudaPeekAtLastError() );
...
// #endif
	...
	while (currentOffset + fullMatrixSize*2 < availableDeviceMemory) {
		if (generate) {
			if (testOffsetWithValues(currentOffset, fullMatrixSize))
				break;
		} else {
...
//================== Init logs
	...
	snprintf(test_name, 50, "cuda_%s_gemm_badmem", test_precision_description);
	printf("%s\n%s\n", test_name, test_info);
#ifdef LOGS
	if (!generate) {
