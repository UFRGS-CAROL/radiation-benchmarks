...
void validate_yolo(char *cfgfile, char *weightfile) {
	...
	for (i = nthreads; i < m + nthreads; i += nthreads) {
		...
		for (t = 0; t < nthreads && i + t - nthreads < m; ++t) {
			...
			if (nms)
				do_nms_sort(boxes, probs, l.side * l.side * l.n, classes,
						iou_thresh);
			print_yolo_detections(fps, id, boxes, probs, l.side * l.side * l.n,
					classes, w, h);
			free(id);
			free_image(val[t]);
