...
bool online_precision_recall(std::vector<box> gold, std::vector<box> found,
		float threshold) {
	float true_positive = 0;
	for (unsigned i = 0; i < gold.size(); i++) {
		//float x, y, w, h;
		float x_max_gold = gold[i].x;
		float y_max_gold = gold[i].y;
		float x_min_gold = x_max_gold - gold[i].w;
		float y_min_gold = y_max_gold - gold[i].h;

		for (unsigned z = 0; z < found.size(); z++) {
			float x_max_found = found[z].x;
			float y_max_found = found[z].y;
			float x_min_found = x_max_found - found[z].w;
			float y_min_found = y_max_found - found[z].h;
...
//			}
	...
	bool online_pr = online_precision_recall(gold_boxes_vector, found_boxes_vector,
			PR_THRESHOLD);

	if(error_count && save_layers && online_pr) {
