...
softmax_layer make_softmax_layer(int batch, int inputs, int groups) {
	...
	l.inputs = inputs;
	l.outputs = inputs;
	l.loss = (real_t*) calloc(inputs * batch, sizeof(real_t));
	l.output = (real_t*) calloc(inputs * batch, sizeof(real_t));
	l.delta = (real_t*) calloc(inputs * batch, sizeof(real_t));
...
void forward_softmax_layer_gpu(const softmax_layer l, network net) {
	if (l.softmax_tree) {
		softmax_tree(net.input_gpu, 1, l.batch, l.inputs,
				double(l.temperature), l.output_gpu, *l.softmax_tree);
		/*
		 int i;
		 int count = 0;
		 for (i = 0; i < l.softmax_tree->groups; ++i) {
		...
		if (l.spatial) {
			...
			softmax_gpu(net.input_gpu, l.inputs / l.groups, l.batch, l.inputs,
					l.groups, l.inputs / l.groups, 1, double(l.temperature),
					l.output_gpu);
		}
	}
	if (net.truth && !l.noloss) {
		...
		if (l.softmax_tree) {
			mask_gpu(l.batch * l.inputs, l.delta_gpu, (SECRET_NUM),
					net.truth_gpu, 0);
			mask_gpu(l.batch * l.inputs, l.loss_gpu, (SECRET_NUM),
					net.truth_gpu, 0);
		}
...
void backward_softmax_layer_gpu(const softmax_layer layer, network net) {
	axpy_gpu(layer.batch * layer.inputs, 1, layer.delta_gpu, 1,
			net.delta_gpu, 1);
}
