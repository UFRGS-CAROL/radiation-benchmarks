...
layer make_reorg_layer(int batch, int w, int h, int c, int stride, int reverse,
	...
	l.output = (real_t*) calloc(output_size, sizeof(real_t));
	l.delta = (real_t*) calloc(output_size, sizeof(real_t));
...
void resize_reorg_layer(layer *l, int w, int h) {
	...
	l->output = (real_t*) realloc(l->output, output_size * sizeof(real_t));
...
void backward_reorg_layer(const layer l, network net) {
	int i;
	if (l.flatten) {
		memcpy(net.delta, l.delta, l.outputs * l.batch * sizeof(real_t));
		if (l.reverse) {
...
void forward_reorg_layer_gpu(layer l, network net) {
	...
	if (l.flatten) {
		if (l.reverse) {
			flatten_gpu(net.input_gpu, l.w * l.h, l.c, l.batch, 0,
					l.output_gpu);
		} else {
			flatten_gpu(net.input_gpu, l.w * l.h, l.c, l.batch, 1,
					l.output_gpu);
		}
	} else if (l.extra) {
		for (i = 0; i < l.batch; ++i) {
