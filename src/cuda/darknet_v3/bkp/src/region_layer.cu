...
layer make_region_layer(int batch, int w, int h, int n, int classes,
	...
	l.biases = (real_t*) calloc(n * 2, sizeof(real_t));
...
box get_region_box(real_t *x, real_t *biases, int n, int index, int i, int j,
...
real_t delta_region_box(box truth, real_t *x, real_t *biases, int n, int index,
		int i, int j, int w, int h, real_t *delta, real_t scale, int stride) {
	box pred = get_region_box(x, biases, n, index, i, j, w, h, stride);
	real_t iou = box_iou(pred, truth);

	real_t tx = real_t(truth.x * w - i);
	real_t ty = real_t(truth.y * h - j);
...
void delta_region_mask(real_t *truth, real_t *x, int n, int index,
		real_t *delta, int stride, int scale) {
	int i;
	for (i = 0; i < n; ++i) {
...
void delta_region_class(real_t *output, real_t *delta, int index, int class_,
		int classes, tree *hier, real_t scale, int stride, real_t *avg_cat,
		int tag) {
	int i, n;
	if (hier) {
		real_t pred = real_t(1);
		while (class_ >= 0) {
			...
			delta[index + stride * class_] = scale
					* (1 - output[index + stride * class_]);
		...
		*avg_cat += pred;
		...
		for (n = 0; n < classes; ++n) {
			delta[index + stride * n] = scale
					* (((n == class_) ? 1 : 0) - output[index + stride * n]);
			if (n == class_)
	...
	for (b = 0; b < l.batch; ++b) {
		if (l.softmax_tree) {
			...
			for (t = 0; t < 30; ++t) {
				...
				if (!truth.x)
					break;
				int class_ = net.truth[t * (l.coords + 1) + b * l.truths
						+ l.coords];
				real_t maxp = real_t(0);
				int maxi = 0;
				if (truth.x > 100000 && truth.y > 100000) {
					...
					delta_region_class(l.output, l.delta, class_index, class_,
				...
				for (n = 0; n < l.n; ++n) {
					...
					if (*(net.seen) < 12800) {
						...
						delta_region_box(truth, l.output, l.biases, n,
								box_index, i, j, l.w, l.h, l.delta, real_t(.01),
								l.w * l.h);
					}
				}
			}
		...
		for (t = 0; t < 30; ++t) {
			...
			int box_index = entry_index(l, b, best_n * l.w * l.h + j * l.w + i,
					0);
			real_t iou = delta_region_box(truth, l.output, l.biases, best_n,
					box_index, i, j, l.w, l.h, l.delta,
					real_t(l.coord_scale * (2 - truth.w * truth.h)), l.w * l.h);
			if (l.coords > 4) {
...
void get_region_detections(layer l, int w, int h, int netw, int neth,
	...
	for (i = 0; i < l.w * l.h; ++i) {
		...
		for (n = 0; n < l.n; ++n) {
			...
			real_t scale = l.background ? real_t(1) : predictions[obj_index];
