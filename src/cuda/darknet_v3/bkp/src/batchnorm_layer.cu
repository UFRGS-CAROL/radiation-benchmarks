...
layer make_batchnorm_layer(int batch, int w, int h, int c) {
	...
	l.w = l.out_w = w;
	l.c = l.out_c = c;
	l.output = (real_t*) calloc(h * w * c * batch, sizeof(real_t));
	l.delta = (real_t*) calloc(h * w * c * batch, sizeof(real_t));
	l.inputs = w * h * c;
	l.outputs = l.inputs;

	l.scales = (real_t*) calloc(c, sizeof(real_t));
	l.scale_updates = (real_t*) calloc(c, sizeof(real_t));
	l.biases = (real_t*) calloc(c, sizeof(real_t));
	l.bias_updates = (real_t*) calloc(c, sizeof(real_t));
	int i;
	for (i = 0; i < c; ++i) {
		l.scales[i] = 1;
	}

	l.mean = (real_t*) calloc(c, sizeof(real_t));
	l.variance = (real_t*) calloc(c, sizeof(real_t));

	l.rolling_mean = (real_t*) calloc(c, sizeof(real_t));
	l.rolling_variance = (real_t*) calloc(c, sizeof(real_t));
...
void variance_delta_cpu(real_t *x, real_t *delta, real_t *mean,
	...
	for (i = 0; i < filters; ++i) {
		...
		variance_delta[i] *= -.5
				* pow(variance[i] + .00001f, (real_t)(-3. / 2.));
	}
}
...
void forward_batchnorm_layer(layer l, network net) {
	...
	if (net.train) {
		...
		scal_cpu(l.out_c, real_t(.99), l.rolling_mean, 1);
		axpy_cpu(l.out_c, real_t(.01), l.mean, 1, l.rolling_mean, 1);
		scal_cpu(l.out_c, real_t(.99), l.rolling_variance, 1);
		axpy_cpu(l.out_c, real_t(.01), l.variance, 1, l.rolling_variance, 1);
