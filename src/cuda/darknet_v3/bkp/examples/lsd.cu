...
/*
 ...
 real_t *X = calloc(x_size, sizeof(real_t));
 real_t *y = calloc(y_size, sizeof(real_t));
 ...
 *gnet->seen += gnet->batch;
 ...
 real_t *feats = fnet->layers[fnet->n - 2].output_gpu;
 copy_gpu(y_size, feats, 1, fstate.truth, 1);

 forward_network_gpu(gnet, gstate);
 real_t *gen = gnet->layers[gnet->n-1].output_gpu;
 ...
 real_t *delta = imlayer.delta_gpu;
 ...
 */
/*
 image sim = real_t_to_image(anet->w, anet->h, anet->c, style.X.vals[j]);
...
/*
 ...
 real_t aloss = train_network(anet, merge);
...
/*
 ...
 gstate.delta = 0;
 gstate.train = 1;
 real_t *pixs = calloc(x_size, sizeof(real_t));
 real_t *graypixs = calloc(x_size, sizeof(real_t));
 real_t *y = calloc(y_size, sizeof(real_t));
 ...
 real_t *imerror = cuda_make_array(0, imlayer.outputs);
 real_t *ones_gpu = cuda_make_array(0, ay_size);
 ...
 for(j = 0; j < imgs; ++j){
 image gim = real_t_to_image(net->w, net->h, net->c, gray.X.vals[j]);
 grayscale_image_3c(gim);
 train.y.vals[j][0] = .9;

 image yim = real_t_to_image(net->w, net->h, net->c, train.X.vals[j]);
 //rgb_to_yuv(yim);
 }
 time=clock();
 real_t gloss = 0;
 ...
 */
/*
 image origi = real_t_to_image(net->w, net->h, 3, pixs);
 image grayi = real_t_to_image(net->w, net->h, 3, graypixs);
...
void slerp(real_t *start, real_t *end, real_t s, int n, real_t *out) {
	real_t omega = acos(dot_cpu(n, start, 1, end, 1));
	real_t so = sin(omega);
	fill_cpu(n, (real_t) 0, out, (real_t) 1);
	axpy_cpu(n, sin(((real_t) 1 - s) * omega) / so, start, (real_t) 1, out,
			(real_t) 1);
	axpy_cpu(n, sin(s * omega) / so, end, (real_t) 1, out, (real_t) 1);

	real_t mag = mag_array(out, n);
	scale_array(out, n, (real_t) 1. / mag);
}
...
image random_unit_vector_image(int w, int h, int c) {
	...
	scale_array(im.data, im.w * im.h * im.c, (real_t) 1. / mag);
	return im;
}
...
void inter_dcgan(char *cfgfile, char *weightfile) {
	...
	while (1) {
		...
		slerp(start.data, end.data, real_t(count / max_count),
...
void set_network_alpha_beta(network *net, real_t alpha, real_t beta) {
	int i;
	for (i = 0; i < net->n; ++i) {
...
void train_prog(char *cfg, char *weight, char *acfg, char *aweight, int clear,
	...
	real_t_device *imerror = cuda_make_array(0, y_size);

	real_t aloss_avg = real_t(-1);

	if (maxbatch == 0)
		maxbatch = gnet->max_batches;
	while (get_current_batch(gnet) < maxbatch) {
		{
			int cb = get_current_batch(gnet);
			real_t alpha = real_t(cb / (maxbatch / 2));
			if (alpha > 1)
				alpha = real_t(1);
			real_t beta = real_t(1) - alpha;
			printf("%f %f\n", alpha, beta);
		...
		for (j = 0; j < gnet->subdivisions; ++j) {
			...
			/*
			 ...
			 real_t mag = mag_array(gnet->input + z*gnet->inputs, gnet->inputs);
			...
			*gnet->seen += gnet->batch;
			...
			fill_gpu(imlayer.outputs * imlayer.batch, real_t(0), imerror, 1);
			fill_cpu(anet->truths * anet->batch, real_t(1), anet->truth, 1);
			copy_cpu(anet->inputs * anet->batch, imlayer.output, 1, anet->input,
					1);
			anet->delta_gpu = imerror;
			forward_network(anet);
			backward_network(anet);

			//real_t genaloss = *anet->cost / anet->batch;

			scal_gpu(imlayer.outputs * imlayer.batch, real_t(1), imerror, 1);
			scal_gpu(imlayer.outputs * imlayer.batch, real_t(0),
					gnet->layers[gnet->n - 1].delta_gpu, 1);

			axpy_gpu(imlayer.outputs * imlayer.batch, real_t(1), imerror, 1,
					gnet->layers[gnet->n - 1].delta_gpu, 1);
			...
			for (k = 0; k < gnet->batch; ++k) {
				int index = j * gnet->batch + k;
				copy_cpu(gnet->outputs, gnet->output + k * gnet->outputs, 1,
						gen.X.vals[index], 1);
			}
		}
		harmless_update_network_gpu(anet);
...
#ifdef OPENCV
		if(display) {
			image im = real_t_to_image(anet->w, anet->h, anet->c, gen.X.vals[0]);
			image im2 = real_t_to_image(anet->w, anet->h, anet->c, train.X.vals[0]);
			show_image(im, "gen", 1);
		...
		printf("%d: adv: %f | adv_avg: %f, %f rate, %lf seconds, %d images\n",
				i, aloss, aloss_avg, get_current_rate(gnet),
...
void train_dcgan(char *cfg, char *weight, char *acfg, char *aweight, int clear,
	...
	network *gnet = load_network(cfg, weight, clear);
	network *anet = load_network(acfg, aweight, clear);
	//real_t orig_rate = anet->learning_rate;
	...
	int x_size = gnet->inputs * gnet->batch;
	int y_size = gnet->truths * gnet->batch;
	real_t *imerror = cuda_make_array(0, y_size);
	...
	while (get_current_batch(gnet) < maxbatch) {
		...
		for (j = 0; j < gnet->subdivisions; ++j) {
			...
			for (z = 0; z < gnet->batch; ++z) {
				real_t mag = mag_array(gnet->input + z * gnet->inputs,
						gnet->inputs);
				scale_array(gnet->input + z * gnet->inputs, gnet->inputs,
						real_t(1. / mag));
			}
			/*
			 for(z = 0; z < 100; ++z){
...
void train_colorizer(char *cfg, char *weight, char *acfg, char *aweight,
	...
	net->delta = 0;
	net->train = 1;
	real_t *pixs = (real_t*) calloc(x_size, sizeof(real_t));
	real_t *graypixs = (real_t*) calloc(x_size, sizeof(real_t));
	//real_t *y = calloc(y_size, sizeof(real_t));
	...
	real_t *imerror = cuda_make_array(0, imlayer.outputs * imlayer.batch);

	real_t aloss_avg = real_t(-1);
	real_t gloss_avg = real_t(-1);
	...
	while (get_current_batch(net) < net->max_batches) {
		...
		for (j = 0; j < net->subdivisions; ++j) {
			...
			*net->seen += net->batch;
			...
			fill_gpu(imlayer.outputs * imlayer.batch, real_t(0), imerror, 1);
			copy_gpu(anet->inputs * anet->batch, imlayer.output_gpu, real_t(1),
					anet->input_gpu, 1);
			fill_gpu(anet->inputs * anet->batch, real_t(.95), anet->truth_gpu, 1);
			anet->delta_gpu = imerror;
			forward_network_gpu(anet);
			backward_network_gpu(anet);

			scal_gpu(imlayer.outputs * imlayer.batch, real_t(1. / 100.),
					net->layers[net->n - 1].delta_gpu, 1);
			...
			printf("realness %f\n",
					cuda_mag_array(imerror, imlayer.outputs * imlayer.batch));
			printf("features %f\n",
					cuda_mag_array(net->layers[net->n - 1].delta_gpu,
							imlayer.outputs * imlayer.batch));
			...
			for (k = 0; k < net->batch; ++k) {
				int index = j * net->batch + k;
				copy_cpu(imlayer.outputs, imlayer.output + k * imlayer.outputs,
						1, gray.X.vals[index], 1);
			}
		}
		harmless_update_network_gpu(anet);
...
#ifdef OPENCV
		if(display) {
			image im = real_t_to_image(anet->w, anet->h, anet->c, gray.X.vals[0]);
			image im2 = real_t_to_image(anet->w, anet->h, anet->c, train.X.vals[0]);
			show_image(im, "gen", 1);
			show_image(im2, "train", 1);
		}
		...
		printf(
				...
				i, gloss, aloss, gloss_avg, aloss_avg, get_current_rate(net),
				sec(clock() - time), i * imgs);
		if (i % 1000 == 0) {
			char buff[256];
			sprintf(buff, "%s/%s_%d.weights", backup_directory, base, i);
