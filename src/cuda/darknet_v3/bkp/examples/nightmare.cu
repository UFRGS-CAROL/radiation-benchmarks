...
void optimize_picture(network *net, image orig, int max_layer, real_t scale,
		real_t rate, real_t thresh, int norm) {
	//scale_image(orig, 2);
...
void reconstruct_picture(network *net, real_t *features, image recon,
		image update, real_t rate, real_t momentum, real_t lambda, int smooth_size,
		int iters) {
	int iter = 0;
