...
layer make_normalization_layer(int batch, int w, int h, int c, int size, real_t alpha, real_t beta, real_t kappa)
{
    ...
    layer.alpha = alpha;
    layer.beta = beta;
    layer.output = calloc(h * w * c * batch, sizeof(real_t));
    layer.delta = calloc(h * w * c * batch, sizeof(real_t));
    layer.squared = calloc(h * w * c * batch, sizeof(real_t));
    layer.norms = calloc(h * w * c * batch, sizeof(real_t));
    layer.inputs = w*h*c;
...
void resize_normalization_layer(layer *layer, int w, int h)
{
    ...
    layer->inputs = w*h*c;
    layer->outputs = layer->inputs;
    layer->output = realloc(layer->output, h * w * c * batch * sizeof(real_t));
    layer->delta = realloc(layer->delta, h * w * c * batch * sizeof(real_t));
    layer->squared = realloc(layer->squared, h * w * c * batch * sizeof(real_t));
    layer->norms = realloc(layer->norms, h * w * c * batch * sizeof(real_t));
#ifdef GPU
...
void forward_normalization_layer_gpu(const layer layer, network net)
{
    ...
    for(b = 0; b < layer.batch; ++b){
        real_t *squared = layer.squared_gpu + w*h*c*b;
        real_t *norms   = layer.norms_gpu + w*h*c*b;
        real_t *input   = net.input_gpu + w*h*c*b;
