...
matrix resize_matrix(matrix m, int size) {
	...
	if (m.rows == size)
		return m;
	if (m.rows < size) {
		m.vals = (real_t**) realloc(m.vals, size * sizeof(real_t*));
		for (i = m.rows; i < size; ++i) {
			m.vals[i] = (real_t*) calloc(m.cols, sizeof(real_t));
		}
	} else if (m.rows > size) {
...
matrix copy_matrix(matrix m) {
	...
	c.rows = m.rows;
	c.cols = m.cols;
	c.vals = (real_t**) calloc(c.rows, sizeof(real_t *));
	int i;
	for (i = 0; i < c.rows; ++i) {
		c.vals[i] = (real_t*) calloc(c.cols, sizeof(real_t));
...
matrix make_matrix(int rows, int cols) {
	...
	m.rows = rows;
	m.cols = cols;
	m.vals = (real_t**) calloc(m.rows, sizeof(real_t *));
	for (i = 0; i < m.rows; ++i) {
		m.vals[i] = (real_t*) calloc(m.cols, sizeof(real_t));
	}
	return m;
...
matrix hold_out_matrix(matrix *m, int n) {
	...
	h.rows = n;
	h.cols = m->cols;
	h.vals = (real_t**) calloc(h.rows, sizeof(real_t *));
	for (i = 0; i < n; ++i) {
...
real_t *pop_column(matrix *m, int c) {
	real_t *col = (real_t*) calloc(m->rows, sizeof(real_t));
	int i, j;
	for (i = 0; i < m->rows; ++i) {
		col[i] = m->vals[i][c];
...
matrix csv_to_matrix(char *filename) {
	...
	int n = 0;
	int size = 1024;
	m.vals = (real_t**) calloc(size, sizeof(real_t*));
	while ((line = fgetl(fp))) {
		if (m.cols == -1)
	...
	m.vals = (real_t**) realloc(m.vals, n * sizeof(real_t*));
	m.rows = n;
	return m;
}
