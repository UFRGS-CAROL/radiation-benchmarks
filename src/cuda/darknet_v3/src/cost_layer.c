...
cost_layer make_cost_layer(int batch, int inputs, COST_TYPE cost_type, real_t scale)
{
    ...
    l.delta = calloc(inputs*batch, sizeof(real_t));
    l.output = calloc(inputs*batch, sizeof(real_t));
...
void resize_cost_layer(cost_layer *l, int inputs)
{
    l->inputs = inputs;
    l->outputs = inputs;
    l->delta = realloc(l->delta, inputs*l->batch*sizeof(real_t));
    l->output = realloc(l->output, inputs*l->batch*sizeof(real_t));
#ifdef GPU
...
int real_t_abs_compare (const void * a, const void * b)
{
    real_t fa = *(const real_t*) a;
    if(fa < 0) fa = -fa;
    real_t fb = *(const real_t*) b;
    if(fb < 0) fb = -fb;
    return (fa > fb) - (fa < fb);
}
...
void forward_cost_layer_gpu(cost_layer l, network net)
{
    ...
    if(l.ratio){
        cuda_pull_array(l.delta_gpu, l.delta, l.batch*l.inputs);
        qsort(l.delta, l.batch*l.inputs, sizeof(real_t), real_t_abs_compare);
