...
layer make_gru_layer(int batch, int inputs, int outputs, int steps, int batch_normalize, int adam)
{
    ...
    *(l.wh) = make_connected_layer(batch*steps, outputs, outputs, LINEAR, batch_normalize, adam);
    ...
    l.delta = calloc(outputs*batch*steps, sizeof(real_t));
    l.state = calloc(outputs*batch, sizeof(real_t));
    l.prev_state = calloc(outputs*batch, sizeof(real_t));
    l.forgot_state = calloc(outputs*batch, sizeof(real_t));
    l.forgot_delta = calloc(outputs*batch, sizeof(real_t));

    l.r_cpu = calloc(outputs*batch, sizeof(real_t));
    l.z_cpu = calloc(outputs*batch, sizeof(real_t));
    l.h_cpu = calloc(outputs*batch, sizeof(real_t));
...
void backward_gru_layer_gpu(layer l, network net)
{
    ...
    for (i = l.steps-1; i >= 0; --i) {
        ...
        real_t *prev_delta_gpu = (i == 0) ? 0 : l.delta_gpu - l.outputs*l.batch;
