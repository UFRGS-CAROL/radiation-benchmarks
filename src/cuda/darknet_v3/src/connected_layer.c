...
layer make_connected_layer(int batch, int inputs, int outputs,
	...
	l.output = calloc(batch * outputs, sizeof(real_t));
	l.delta = calloc(batch * outputs, sizeof(real_t));

	l.weight_updates = calloc(inputs * outputs, sizeof(real_t));
	l.bias_updates = calloc(outputs, sizeof(real_t));

	l.weights = calloc(outputs * inputs, sizeof(real_t));
	l.biases = calloc(outputs, sizeof(real_t));
	...
	if (adam) {
		l.m = calloc(l.inputs * l.outputs, sizeof(real_t));
		l.v = calloc(l.inputs * l.outputs, sizeof(real_t));
		l.bias_m = calloc(l.outputs, sizeof(real_t));
		l.scale_m = calloc(l.outputs, sizeof(real_t));
		l.bias_v = calloc(l.outputs, sizeof(real_t));
		l.scale_v = calloc(l.outputs, sizeof(real_t));
	}
	if (batch_normalize) {
		l.scales = calloc(outputs, sizeof(real_t));
		l.scale_updates = calloc(outputs, sizeof(real_t));
		for (i = 0; i < outputs; ++i) {
			l.scales[i] = 1;
		}

		l.mean = calloc(outputs, sizeof(real_t));
		l.mean_delta = calloc(outputs, sizeof(real_t));
		l.variance = calloc(outputs, sizeof(real_t));
		l.variance_delta = calloc(outputs, sizeof(real_t));

		l.rolling_mean = calloc(outputs, sizeof(real_t));
		l.rolling_variance = calloc(outputs, sizeof(real_t));

		l.x = calloc(batch * outputs, sizeof(real_t));
		l.x_norm = calloc(batch * outputs, sizeof(real_t));
	}
...
void forward_connected_layer_gpu(layer l, network net) {
	...
	gemm_gpu(0, 1, m, n, k, 1, a, k, b, k, 1, c, n, net.use_tensor_cores);
...
void backward_connected_layer_gpu(layer l, network net) {
	...
	gemm_gpu(1, 0, m, n, k, 1, a, m, b, n, 1, c, n, net.use_tensor_cores);
	...
	if (c)
		gemm_gpu(0, 0, m, n, k, 1, a, k, b, n, 1, c, n, net.use_tensor_cores);
}
#endif
