...
void train_compare(char *cfgfile, char *weightfile)
{
    ...
    while(1){
        ...
        printf("%.3f: %f, %f avg, %lf seconds, %ld images\n", (real_t)*net.seen/N, loss, avg_loss, sec(clock()-time), *net.seen);
        free_data(train);
        if(i%100 == 0){
            char buff[256];
...
void validate_compare(char *filename, char *weightfile)
{
    ...
    for(i = 1; i <= splits; ++i){
        ...
        free_matrix(pred);
        printf("%d: Acc: %f, %lf seconds, %d images\n", i, (real_t)correct/total, sec(clock()-time), val.X.rows);
        free_data(val);
    }
}
...
int bbox_comparator(const void *a, const void *b)
{
    ...
    real_t *X  = calloc(net.w*net.h*net.c, sizeof(real_t));
    memcpy(X,                   im1.data, im1.w*im1.h*im1.c*sizeof(real_t));
    memcpy(X+im1.w*im1.h*im1.c, im2.data, im2.w*im2.h*im2.c*sizeof(real_t));
...
{
    int k = 32;
    real_t EA = 1./(1+pow(10, (b->elos[class] - a->elos[class])/400.));
    real_t EB = 1./(1+pow(10, (a->elos[class] - b->elos[class])/400.));
    real_t SA = result ? 1 : 0;
    real_t SB = result ? 0 : 1;
...
void BattleRoyaleWithCheese(char *filename, char *weightfile)
{
    ...
    for(i = 0; i < N; ++i){
        ...
        boxes[i].elos = calloc(classes, sizeof(real_t));;
        for(j = 0; j < classes; ++j){
            boxes[i].elos[j] = 1500;
        }
    }
