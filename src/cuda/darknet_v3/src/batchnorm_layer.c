...
layer make_batchnorm_layer(int batch, int w, int h, int c)
{
    ...
    l.w = l.out_w = w;
    l.c = l.out_c = c;
    l.output = calloc(h * w * c * batch, sizeof(real_t));
    l.delta  = calloc(h * w * c * batch, sizeof(real_t));
    l.inputs = w*h*c;
    l.outputs = l.inputs;

    l.scales = calloc(c, sizeof(real_t));
    l.scale_updates = calloc(c, sizeof(real_t));
    l.biases = calloc(c, sizeof(real_t));
    l.bias_updates = calloc(c, sizeof(real_t));
    int i;
    for(i = 0; i < c; ++i){
        l.scales[i] = 1;
    }

    l.mean = calloc(c, sizeof(real_t));
    l.variance = calloc(c, sizeof(real_t));

    l.rolling_mean = calloc(c, sizeof(real_t));
    l.rolling_variance = calloc(c, sizeof(real_t));
...
void  variance_delta_cpu(real_t *x, real_t *delta, real_t *mean, real_t *variance, int batch, int filters, int spatial, real_t *variance_delta)
{
    ...
    for(i = 0; i < filters; ++i){
        ...
        variance_delta[i] *= -.5 * pow(variance[i] + .00001f, (real_t)(-3./2.));
    }
}
