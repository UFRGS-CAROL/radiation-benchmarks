...
layer make_gru_layer(int batch, int inputs, int outputs, int steps,
	...
	*(l.wh) = make_connected_layer(batch * steps, outputs, outputs, LINEAR,
	...
	l.outputs = outputs;
	l.output = (real_t*) calloc(outputs * batch * steps, sizeof(real_t));
	l.delta = (real_t*) calloc(outputs * batch * steps, sizeof(real_t));
	l.state = (real_t*) calloc(outputs * batch, sizeof(real_t));
	l.prev_state = (real_t*) calloc(outputs * batch, sizeof(real_t));
	l.forgot_state = (real_t*) calloc(outputs * batch, sizeof(real_t));
	l.forgot_delta = (real_t*) calloc(outputs * batch, sizeof(real_t));

	l.r_cpu = (real_t*) calloc(outputs * batch, sizeof(real_t));
	l.z_cpu = (real_t*) calloc(outputs * batch, sizeof(real_t));
	l.h_cpu = (real_t*) calloc(outputs * batch, sizeof(real_t));
...
void forward_gru_layer(layer l, network net) {
	...
	fill_cpu(l.outputs * l.batch * l.steps, real_t(0), uz.delta, 1);
	fill_cpu(l.outputs * l.batch * l.steps, real_t(0), ur.delta, 1);
	fill_cpu(l.outputs * l.batch * l.steps, real_t(0), uh.delta, 1);

	fill_cpu(l.outputs * l.batch * l.steps, real_t(0), wz.delta, 1);
	fill_cpu(l.outputs * l.batch * l.steps, real_t(0), wr.delta, 1);
	fill_cpu(l.outputs * l.batch * l.steps, real_t(0), wh.delta, 1);
	if (net.train) {
		fill_cpu(l.outputs * l.batch * l.steps, real_t(0), l.delta, 1);
		copy_cpu(l.outputs * l.batch, l.state, real_t(1), l.prev_state, 1);
	}
	...
	for (i = 0; i < l.steps; ++i) {
		...
		axpy_cpu(l.outputs * l.batch, real_t(1), wz.output, 1, l.z_cpu, 1);

		copy_cpu(l.outputs * l.batch, ur.output, 1, l.r_cpu, 1);
		axpy_cpu(l.outputs * l.batch, real_t(1), wr.output, 1, l.r_cpu, 1);
		...
		axpy_cpu(l.outputs * l.batch, real_t(1), wh.output, 1, l.h_cpu, 1);
...
void backward_gru_layer_gpu(layer l, network net) {
	...
	real_t_device *end_state = l.output_gpu;
	for (i = l.steps - 1; i >= 0; --i) {
		...
		weighted_delta_gpu(l.state_gpu, l.h_gpu, l.z_gpu, prev_delta_gpu,
				uh.delta_gpu, uz.delta_gpu, l.outputs * l.batch, l.delta_gpu);
