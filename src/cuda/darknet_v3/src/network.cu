...
real_t get_current_rate(network *net) {
	...
	if (batch_num < net->burn_in)
		return real_t(
				net->learning_rate
						* pow((real_t) batch_num / net->burn_in, net->power));
	switch (net->policy) {
	case CONSTANT:
		return net->learning_rate;
	case STEP:
		return net->learning_rate
				* real_t(pow(net->scale, batch_num / net->step));
	case STEPS:
		...
		return rate;
	case EXP:
		return net->learning_rate * real_t(pow(net->gamma, batch_num));
	case POLY:
		return net->learning_rate
				* real_t(
						pow(1 - (real_t) batch_num / net->max_batches,
								net->power));
	case RANDOM:
		return net->learning_rate
				* real_t(pow(rand_uniform(real_t(0), real_t(1)), net->power));
	case SIG:
		return net->learning_rate
				* real_t(
...
network *make_network(int n) {
	...
	net->cost = (real_t*) calloc(1, sizeof(real_t));
	return net;
}
...
void forward_network(network *netp) {
	...
	for (i = 0; i < net.n; ++i) {
		...
		layer l = net.layers[i];
		if (l.delta) {
			fill_cpu(l.outputs * l.batch, real_t(0), l.delta, 1);
		}
		l.forward(l, net);
		net.input = l.output;
		if (l.truth) {
...
real_t train_network_sgd(network *net, data d, int n) {
	...
	for (i = 0; i < n; ++i) {
		...
		real_t err = train_network_datum(net);
		sum += err;
	}
	return real_t(sum / (n * batch));
}
...
image get_network_image_layer(network *net, int i) {
	...
	if (l.out_w && l.out_h && l.out_c) {
		return real_t_to_image(l.out_w, l.out_h, l.out_c, l.output);
	}
	image def = { 0 };
	return def;
}
...
detection *make_network_boxes(network *net, real_t thresh, int *num) {
...
void fill_network_boxes(network *net, int w, int h, real_t thresh, real_t hier,
...
real_t *network_predict_image(network *net, image im) {
	...
	real_t *p = network_predict(net, imr.data);
	free_image(imr);
	return p;
}
...
matrix network_predict_data_multi(network *net, data test, int n) {
	...
	real_t *X = (real_t*) calloc(net->batch * test.X.rows, sizeof(real_t));
	for (i = 0; i < test.X.rows; i += net->batch) {
		for (b = 0; b < net->batch; ++b) {
			if (i + b == test.X.rows)
				break;
			memcpy(X + b * test.X.cols, test.X.vals[i + b],
					test.X.cols * sizeof(real_t));
		}
		for (m = 0; m < n; ++m) {
			real_t *out = network_predict(net, X);
			for (b = 0; b < net->batch; ++b) {
				if (i + b == test.X.rows)
					break;
				for (j = 0; j < k; ++j) {
...
matrix network_predict_data(network *net, data test) {
	...
	int k = net->outputs;
	matrix pred = make_matrix(test.X.rows, k);
	real_t *X = (real_t*) calloc(net->batch * test.X.cols, sizeof(real_t));
	for (i = 0; i < test.X.rows; i += net->batch) {
		for (b = 0; b < net->batch; ++b) {
			if (i + b == test.X.rows)
				break;
...
void print_network(network *net) {
	...
	for (i = 0; i < net->n; ++i) {
		layer l = net->layers[i];
		real_t *output = l.output;
		int n = l.outputs;
		real_t mean = mean_array(output, n);
		real_t vari = variance_array(output, n);
...
void compare_networks(network *n1, network *n2, data test) {
	...
	printf("%5d %5d\n%5d %5d\n", a, b, c, d);
	real_t num = real_t(pow((abs(b - c) - 1.), 2.));
	real_t den = real_t(b + c);
	printf("%f\n", num / den);
}

real_t network_accuracy(network *net, data d) {
	matrix guess = network_predict_data(net, d);
	real_t acc = matrix_topk_accuracy(d.y, guess, 1);
	free_matrix(guess);
	return acc;
}
...
real_t *network_output(network *net) {
...
pthread_t train_network_in_thread(network *net, data d, real_t *err) {
	pthread_t thread;
...
void merge_weights(layer l, layer base) {
	if (l.type == CONVOLUTIONAL) {
		axpy_cpu(l.n, real_t(1), l.bias_updates, 1, base.biases, 1);
		axpy_cpu(l.nweights, real_t(1), l.weight_updates, 1, base.weights, 1);
		if (l.scales) {
			axpy_cpu(l.n, real_t(1), l.scale_updates, 1, base.scales, 1);
		}
	} else if (l.type == CONNECTED) {
		axpy_cpu(l.outputs, real_t(1), l.bias_updates, 1, base.biases, 1);
		axpy_cpu(l.outputs * l.inputs, real_t(1), l.weight_updates, 1,
				base.weights, 1);
	}
}

void scale_weights(layer l, real_t s) {
	if (l.type == CONVOLUTIONAL) {
...
/*
 ...
 real_t rate = get_current_rate(net);
...
real_t train_networks(network **nets, int n, data d, int interval) {
	...
	real_t *errors = (real_t *) calloc(n, sizeof(real_t));
