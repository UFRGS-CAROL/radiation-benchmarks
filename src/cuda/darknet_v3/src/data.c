...
matrix load_image_paths_gray(char **paths, int n, int w, int h) {
	...
	matrix X;
	X.rows = n;
	X.vals = calloc(X.rows, sizeof(real_t*));
	X.cols = 0;
...
data load_data_seg(int n, char **paths, int m, int w, int h, int classes,
	...
	d.X.rows = n;
	d.X.vals = calloc(d.X.rows, sizeof(real_t*));
	d.X.cols = h * w * 3;

	d.y.rows = n;
	d.y.cols = h * w * classes / div / div;
	d.y.vals = calloc(d.X.rows, sizeof(real_t*));
...
data load_data_compare(int n, char **paths, int m, int classes, int w, int h) {
	...
	for (i = 0; i < n; ++i) {
		...
		d.X.vals[i] = calloc(d.X.cols, sizeof(real_t));
...
data load_data_super(char **paths, int n, int m, int w, int h, int scale) {
	...
	int i;
	d.X.rows = n;
	d.X.vals = calloc(n, sizeof(real_t*));
	d.X.cols = w * h * 3;

	d.y.rows = n;
	d.y.vals = calloc(n, sizeof(real_t*));
...
data select_data(data *orig, int *inds) {
	...
	d.X.vals = calloc(orig[0].X.rows, sizeof(real_t *));
	d.y.vals = calloc(orig[0].y.rows, sizeof(real_t *));
	int i;
...
matrix concat_matrix(matrix m1, matrix m2) {
	...
	m.vals = calloc(m1.rows + m2.rows, sizeof(real_t*));
	for (i = 0; i < m1.rows; ++i) {
...
data get_random_data(data d, int num) {
	...
	r.X.vals = calloc(num, sizeof(real_t *));
	r.y.vals = calloc(num, sizeof(real_t *));
...
data *split_data(data d, int part, int total) {
	...
	train.X.vals = calloc(train.X.rows, sizeof(real_t*));
	test.X.vals = calloc(test.X.rows, sizeof(real_t*));
	train.y.vals = calloc(train.y.rows, sizeof(real_t*));
	test.y.vals = calloc(test.y.rows, sizeof(real_t*));
