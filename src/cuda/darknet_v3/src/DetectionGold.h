...
public:
	...
	DetectionGold(int argc, char **argv, real_t thresh, real_t hier_thresh) {
		...
		if (this->generate != true) {
			...
			this->app_logging = new Log(this->gold_inout, 0, 0,
					this->iterations, this->network_name,
					this->tensor_core_mode);
			...
			//allocate detector
			this->gold_img_names = std::vector < std::string
					> (this->plist_size);
	...
	void save_gold_img_i(char *img, int num, int classes, real_t **probs,
			box *boxes) {
		ofstream output(this->gold_inout);
	...
	friend ostream& operator<<(ostream& ret, DetectionGold &det) {
