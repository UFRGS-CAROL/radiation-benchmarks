...
void flatten(real_t *x, int size, int layers, int batch, int forward) {
	real_t *swap = (real_t*) calloc(size * layers * batch, sizeof(real_t));
	int i, c, b;
	for (b = 0; b < batch; ++b) {
		for (c = 0; c < layers; ++c) {
			for (i = 0; i < size; ++i) {
	...
	memcpy(x, swap, size * layers * batch * sizeof(real_t));
	free(swap);
}

void weighted_sum_cpu(real_t *a, real_t *b, real_t *s, int n, real_t *c) {
	int i;
	for (i = 0; i < n; ++i) {
...
void softmax(real_t *input, int n, real_t temp, int stride, real_t *output) {
	...
	for (i = 0; i < n; ++i) {
		real_t e = exp(input[i * stride] / temp - largest / temp);
		sum += e;
		output[i * stride] = e;
	}
	for (i = 0; i < n; ++i) {
