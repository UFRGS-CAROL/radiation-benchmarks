...
void forward_detection_layer(const detection_layer l, network net)
{
    ...
    if(net.train){
        ...
        *(l.cost) = 0;
        ...
        memset(l.delta, 0, size * sizeof(real_t));
        for (b = 0; b < l.batch; ++b){
            int index = b*l.inputs;
            for (i = 0; i < locations; ++i) {
                ...
                box truth = real_t_to_box(net.truth + truth_index + 1 + l.classes, 1);
                truth.x /= l.side;
                truth.y /= l.side;

                for(j = 0; j < l.n; ++j){
                    int box_index = index + locations*(l.classes + l.n) + (i*l.n + j) * l.coords;
                    box out = real_t_to_box(l.output + box_index, 1);
                    out.x /= l.side;
                    out.y /= l.side;
                    ...
                    real_t iou  = box_iou(out, truth);
                    //iou = 0;
                    real_t rmse = box_rmse(out, truth);
                    if(best_iou > 0 || iou > 0){
                        if(iou > best_iou){
                            best_iou = iou;
                            best_index = j;
        ...
        if(0){
            real_t *costs = calloc(l.batch*locations*l.n, sizeof(real_t));
            for (b = 0; b < l.batch; ++b) {
                int index = b*l.inputs;
                for (i = 0; i < locations; ++i) {
                    for (j = 0; j < l.n; ++j) {
            ...
            real_t cutoff = costs[indexes[99]];
            for (b = 0; b < l.batch; ++b) {
                int index = b*l.inputs;
                for (i = 0; i < locations; ++i) {
                    for (j = 0; j < l.n; ++j) {
...
void get_detection_detections(layer l, int w, int h, real_t thresh, detection *dets)
{
    ...
    for (i = 0; i < l.side*l.side; ++i){
        ...
        for(n = 0; n < l.n; ++n){
            ...
            real_t scale = predictions[p_index];
            ...
            for(j = 0; j < l.classes; ++j){
                ...
                real_t prob = scale*predictions[class_index+j];
