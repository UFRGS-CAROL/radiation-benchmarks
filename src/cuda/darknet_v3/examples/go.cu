...
static void propagate_liberty(real_t *board, int *lib, int *visited, int row,
		int col, int side) {
...
int compare_board(real_t *a, real_t *b) {
	if (memcmp(a, b, 19 * 19 * 3 * sizeof(real_t)) == 0)
		return 1;
	return 0;
}
...
real_t *network_predict_rotations(network *net, real_t *next) {
	int n = net->batch;
	real_t *in = (real_t*) calloc(19 * 19 * 3 * n, sizeof(real_t));
...
mcts_tree *expand(real_t *next, real_t *ko, network *net) {
	...
	root->prior = (real_t*) calloc(19 * 19 + 1, sizeof(real_t));
	root->prob = (real_t*) calloc(19 * 19 + 1, sizeof(real_t));
	root->mean = (real_t*) calloc(19 * 19 + 1, sizeof(real_t));
	root->value = (real_t*) calloc(19 * 19 + 1, sizeof(real_t));
...
real_t *copy_board(real_t *board) {
	real_t *next = (real_t*) calloc(19 * 19 * 3, sizeof(real_t));
...
mcts_tree *run_mcts(mcts_tree *tree, network *net, real_t *board, real_t *ko,
		int player, int n, real_t cpuct, real_t secs) {
	int i;
...
void valid_go(char *cfgfile, char *weightfile, int multi, char *filename) {
	...
	real_t *board = (real_t*) calloc(19 * 19 * 3, sizeof(real_t));
	real_t *move = (real_t*) calloc(19 * 19 + 2, sizeof(real_t));
...
mcts_tree *ponder(mcts_tree *tree, network *net, real_t *b, real_t *ko,
		int player, real_t cpuct) {
	double t = what_time_is_it_now();
	int count = 0;
	if (tree)
...
void engine_go(char *filename, char *weightfile, int mcts_iters, real_t secs,
		real_t temp, real_t cpuct, int anon, int resign) {
	mcts_tree *root = 0;
	...
	real_t *board = (real_t*) calloc(19 * 19 * 3, sizeof(real_t));
	flip_board(board);
	real_t *one = (real_t*) calloc(19 * 19 * 3, sizeof(real_t));
	real_t *two = (real_t*) calloc(19 * 19 * 3, sizeof(real_t));
	int ponder_player = 0;
	int passed = 0;
	int move_num = 0;
	int main_time = 0;
...
void test_go(char *cfg, char *weights, int multi) {
	...
	real_t *board = (real_t*) calloc(19 * 19 * 3, sizeof(real_t));
	flip_board(board);
	real_t *move = (real_t*) calloc(19 * 19 + 1, sizeof(real_t));
	int color = 1;
	while (1) {
...
real_t score_game(real_t *board) {
	...
	while ((l = fgetl(p))) {
		fprintf(stderr, "%s  \t", l);
		//TODO: make ready for all precisions
		float tmp;
		int n = sscanf(l, "= %c+%f", &player, &tmp);
		score = tmp;
		free(l);
		if (n == 2)
			break;
	...
	while (1) {
		if (done) {
			...
			fprintf(stderr, "Total: %d, Player 1: %f, Player 2: %f\n", total,
					(real_t) p1 / total, (real_t) p2 / total);
			sleep(1);
			/*
			 int i = (score > 0)? 0 : 1;
			 int j;
