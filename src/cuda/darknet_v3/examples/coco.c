...
void train_coco(char *cfgfile, char *weightfile)
{
    ...
    while(get_current_batch(net) < net->max_batches){
        ...
        /*
           image im = real_t_to_image(net->w, net->h, 3, train.X.vals[113]);
...
void validate_coco_recall(char *cfgfile, char *weightfile)
{
    ...
    for(i = 0; i < m; ++i){
        ...
        free_detections(dets, nboxes);
        fprintf(stderr, "%5d %5d %5d\tRPs/Img: %.2f\tIOU: %.2f%%\tRecall:%.2f%%\n", i, correct, total, (real_t)proposals/(i+1), avg_iou*100/total, 100.*correct/total);
        free(id);
        free_image(orig);
        free_image(sized);
    }
...
void test_coco(char *cfgfile, char *weightfile, char *filename, real_t thresh)
{
