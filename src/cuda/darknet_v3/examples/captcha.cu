...
void train_captcha(char *cfgfile, char *weightfile) {
	...
	while (1) {
		...
		/*
		 image im = real_t_to_image(256, 256, 3, train.X.vals[114]);
		...
		if (avg_loss == (real_t) -1)
			avg_loss = loss;
		avg_loss = avg_loss * (real_t).9 + loss * (real_t).1;
...
/*
 ...
 real_t *X = im.data;
 real_t *predictions = network_predict(net, X);
 image out  = real_t_to_image(300, 57, 1, predictions);
 ...
 printf("Word Accuracy: %f, Char Accuracy %f\n", (real_t)accuracy/imgs, (real_t)correct/total);
 free_data(valid);
 }
