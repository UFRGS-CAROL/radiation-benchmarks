...
void calculate_loss(real_t *output, real_t *delta, int n, real_t thresh)
{
    ...
    real_t var = variance_array(output, n);
    for(i = 0; i < n; ++i){
...
void optimize_picture(network *net, image orig, int max_layer, real_t scale, real_t rate, real_t thresh, int norm)
{
    //scale_image(orig, 2);
...
void smooth(image recon, image update, real_t lambda, int num)
{
    int i, j, k;
    int ii, jj;
    for(k = 0; k < recon.c; ++k){
...
void reconstruct_picture(network *net, real_t *features, image recon, image update, real_t rate, real_t momentum, real_t lambda, int smooth_size, int iters)
{
    ...
    for (iter = 0; iter < iters; ++iter) {
        ...
        real_t mag = mag_array(delta.data, recon.w*recon.h*recon.c);
        printf("mag: %f\n", mag);
