...
void train_writing(char *cfgfile, char *weightfile)
{
    ...
    while(get_current_batch(net) < net.max_batches || net.max_batches == 0){
        ...
        /*
           image pred = real_t_to_image(64, 64, 1, out);
           print_image(pred);
         */

        /*
           image im = real_t_to_image(256, 256, 3, train.X.vals[0]);
           image lab = real_t_to_image(64, 64, 1, train.y.vals[0]);
           image pred = real_t_to_image(64, 64, 1, out);
           show_image(im, "image");
        ...
        printf("%ld, %.3f: %f, %f avg, %f rate, %lf seconds, %ld images\n", get_current_batch(net), (real_t)(*net.seen)/N, loss, avg_loss, get_current_rate(net), sec(clock()-time), *net.seen);
        free_data(train);
