...
int legal_go(real_t *b, real_t *ko, int p, int r, int c);
int check_ko(real_t *x, real_t *ko);
...
void string_to_board(char *s, real_t *board)
{
    int i, j;
    memset(board, 0, 2*19*19*sizeof(real_t));
    int count = 0;
    for(i = 0; i < 91; ++i){
        char c = s[i];
        for(j = 0; j < 4; ++j){
...
void board_to_string(char *s, real_t *board)
{
    int i, j;
...
static int occupied(real_t *b, int i)
{
    if (b[i]) return 1;
...
data random_go_moves(moves m, int n)
{
    ...
    int i, j;
    for(i = 0; i < n; ++i){
        real_t *board = d.X.vals[i];
        real_t *label = d.y.vals[i];
        ...
        int flip = rand()%2;
        int rotate = rand()%4;
        image in = real_t_to_image(19, 19, 3, board);
        image out = real_t_to_image(19, 19, 1, label);
        if(flip){
            flip_image(in);
            flip_image(out);
        }
...
void train_go(char *cfgfile, char *weightfile, char *filename, int *gpus, int ngpus, int clear)
{
    ...
    while(get_current_batch(net) < net->max_batches || net->max_batches == 0){
        ...
        printf("%ld, %.3f: %f, %f avg, %f rate, %lf seconds, %ld images\n", get_current_batch(net), (real_t)(*net->seen)/N, loss, avg_loss, get_current_rate(net), what_time_is_it_now()-time, *net->seen);
        if(*net->seen/N > epoch){
            epoch = *net->seen/N;
            char buff[256];
...
static void propagate_liberty(real_t *board, int *lib, int *visited, int row, int col, int side)
{
...
static int *calculate_liberties(real_t *board)
{
...
void print_board(FILE *stream, real_t *board, int player, int *indexes)
{
    int i,j,n;
    fprintf(stream, "   ");
    for(i = 0; i < 19; ++i){
...
real_t predict_move2(network *net, real_t *board, real_t *move, int multi)
{
    real_t *output = network_predict(net, board);
    copy_cpu(19*19+1, output, 1, move, 1);
    real_t result = output[19*19 + 1];
    int i;
    if(multi){
        image bim = real_t_to_image(19, 19, 3, board);
        for(i = 1; i < 8; ++i){
            rotate_image_cw(bim, i);
            if(i >= 4) flip_image(bim);

            real_t *output = network_predict(net, board);
            image oim = real_t_to_image(19, 19, 1, output);
...
static void remove_connected(real_t *b, int *lib, int p, int r, int c)
{
...
void move_go(real_t *b, int p, int r, int c)
{
...
int compare_board(real_t *a, real_t *b)
{
    if(memcmp(a, b, 19*19*3*sizeof(real_t)) == 0) return 1;
    return 0;
}
...
real_t *network_predict_rotations(network *net, real_t *next)
{
    int n = net->batch;
    real_t *in = calloc(19*19*3*n, sizeof(real_t));
    image im = real_t_to_image(19, 19, 3, next);
    int i,j;
    ...
    for(j = 0; j < n; ++j){
        ...
        memcpy(in + 19*19*3*j, im.data, 19*19*3*sizeof(real_t));
        if(i >= 4) flip_image(im);
        rotate_image_cw(im, -i);
    }
    real_t *pred = network_predict(net, in);
    for(j = 0; j < n; ++j){
        i = inds[j];
        image im = real_t_to_image(19, 19, 1, pred + j*(19*19 + 2));
        if(i >= 4) flip_image(im);
        rotate_image_cw(im, -i);
        if(j > 0){
...
mcts_tree *expand(real_t *next, real_t *ko, network *net)
{
    mcts_tree *root = calloc(1, sizeof(mcts_tree));
    root->board = next;
    root->children = calloc(19*19+1, sizeof(mcts_tree*));
    root->prior = calloc(19*19 + 1, sizeof(real_t));
    root->prob = calloc(19*19 + 1, sizeof(real_t));
    root->mean = calloc(19*19 + 1, sizeof(real_t));
    root->value = calloc(19*19 + 1, sizeof(real_t));
...
real_t *copy_board(real_t *board)
{
    real_t *next = calloc(19*19*3, sizeof(real_t));
    copy_cpu(19*19*3, board, 1, next, 1);
    return next;
}

real_t select_mcts(mcts_tree *root, network *net, real_t *prev, real_t cpuct)
{
    ...
    if (root->children[i]) {
        ...
        if(max_i < 19*19 && !legal_go(root->board, prev, 1, max_i/19, max_i%19)) {
            ...
            real_t *next = copy_board(root->board);
            if (max_i < 19*19) {
...
mcts_tree *run_mcts(mcts_tree *tree, network *net, real_t *board, real_t *ko, int player, int n, real_t cpuct, real_t secs)
{
    int i;
...
/*
   real_t predict_move(network *net, real_t *board, real_t *move, int multi, real_t *ko, real_t temp)
   {
   ...
   real_t val = root->result;
   free_mcts(root);
   return val;
   }
 */

static int makes_safe_go(real_t *b, int *lib, int p, int r, int c){
...
int suicide_go(real_t *b, int p, int r, int c)
{
    int *l = calculate_liberties(b);
    int safe = 0;
...
/*
   move generate_move(mcts_tree *root, network *net, int player, real_t *board, int multi, real_t temp, real_t *ko, int print)
   {
   move m = {0};
//root = run_mcts(tree, network *net, real_t *board, real_t *ko, int n, real_t cpuct)
int i, j;
int empty = 1;
for(i = 0; i < 19*19; ++i){
if (occupied(board, i)) {
...
real_t move[362];
if (player < 0) flip_board(board);
real_t result = predict_move(net, board, move, multi, ko, temp);
if (player < 0) flip_board(board);
...
void valid_go(char *cfgfile, char *weightfile, int multi, char *filename)
{
    ...
    real_t *board = calloc(19*19*3, sizeof(real_t));
    real_t *move = calloc(19*19+2, sizeof(real_t));
    ...
    for (i = 0; i <N; ++i) {
        ...
        printf("%d Accuracy %f\n", i, (real_t) correct/(i+1));
    }
}

int print_game(real_t *board, FILE *fp)
{
    int i, j;
    int count = 3;
    fprintf(fp, "komi 6.5\n");
...
mcts_tree *ponder(mcts_tree *tree, network *net, real_t *b, real_t *ko, int player, real_t cpuct)
{
    double t = what_time_is_it_now();
    int count = 0;
...
void engine_go(char *filename, char *weightfile, int mcts_iters, real_t secs, real_t temp, real_t cpuct, int anon, int resign)
{
    mcts_tree *root = 0;
    ...
    real_t *board = calloc(19*19*3, sizeof(real_t));
    flip_board(board);
    real_t *one = calloc(19*19*3, sizeof(real_t));
    real_t *two = calloc(19*19*3, sizeof(real_t));
    int ponder_player = 0;
    int passed = 0;
    int move_num = 0;
    int main_time = 0;
    ...
    while(1){
        ...
        if (!strcmp(buff, "protocol_version")){
            ...
            if(boardsize != 19){
                ...
                memset(board, 0, 3*19*19*sizeof(real_t));
                flip_board(board);
                move_num = 0;
                printf("=%s \n\n", ids);
            }
...
void test_go(char *cfg, char *weights, int multi)
{
    ...
    real_t *board = calloc(19*19*3, sizeof(real_t));
    flip_board(board);
    real_t *move = calloc(19*19+1, sizeof(real_t));
    int color = 1;
    while(1){
        real_t result = predict_move2(net, board, move, multi);
...
real_t score_game(real_t *board)
{
    int i;
    FILE *f = fopen("game.txt", "w");
    int count = print_game(board, f);
