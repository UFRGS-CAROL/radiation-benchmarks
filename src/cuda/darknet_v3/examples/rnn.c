...
real_t_pair get_rnn_token_data(int *tokens, size_t *offsets, int characters, size_t len, int batch, int steps)
{
...
real_t_pair get_seq2seq_data(char **source, char **dest, int n, int characters, size_t len, int batch, int steps)
{
    int i,j;
    real_t *x = calloc(batch * steps * characters, sizeof(real_t));
    real_t *y = calloc(batch * steps * characters, sizeof(real_t));
...
real_t_pair get_rnn_data(unsigned char *text, size_t *offsets, int characters, size_t len, int batch, int steps)
{
    real_t *x = calloc(batch * steps * characters, sizeof(real_t));
    real_t *y = calloc(batch * steps * characters, sizeof(real_t));
    int i,j;
...
void train_char_rnn(char *cfgfile, char *weightfile, char *filename, int clear, int tokenized)
{
    ...
    while(get_current_batch(net) < net->max_batches){
        ...
        fprintf(stderr, "%d: %f, %f avg, %f rate, %lf seconds, %f epochs\n", i, loss, avg_loss, get_current_rate(net), sec(clock()-time), (real_t) chars/size);
...
void test_char_rnn(char *cfgfile, char *weightfile, int num, char *seed, real_t temp, int rseed, char *token_file)
{
    char **tokens = 0;
    if(token_file){
        size_t n;
...
void test_tactic_rnn_multi(char *cfgfile, char *weightfile, int num, real_t temp, int rseed, char *token_file)
{
    char **tokens = 0;
    if(token_file){
        size_t n;
...
void test_tactic_rnn(char *cfgfile, char *weightfile, int num, real_t temp, int rseed, char *token_file)
{
    char **tokens = 0;
    if(token_file){
        size_t n;
