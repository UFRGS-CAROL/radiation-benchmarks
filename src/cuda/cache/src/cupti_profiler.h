#ifndef CUPTI_PROFILER_H_
#define CUPTI_PROFILER_H_
...
get_value_callback(void *userdata, CUpti_CallbackDomain domain,
	...
	std::map<std::string, detail::kernel_data_t> *kernel_data = (std::map<
			std::string, detail::kernel_data_t> *) userdata;
	...
	if (cbInfo->callbackSite == CUPTI_API_ENTER) {
		...
		if (kernel_data->count(current_kernel_name) == 0) {
			...
			for (int i = 0; i < pass_data[0].event_groups->numEventGroups;
					i++) {
		...
		for (int i = 0; i < pass_data.event_groups->numEventGroups; i++) {
			...
			CUPTI_CALL(
					cuptiDeviceGetEventDomainAttribute(current_kernel.m_device,
							group_domain,
			...
			CUPTI_CALL(
					cuptiEventGroupGetAttribute(group,
							CUPTI_EVENT_GROUP_ATTR_NUM_EVENTS, &numEventsSize,
							&numEvents));
			eventIdsSize = numEvents * sizeof(CUpti_EventID);
			eventIds = (CUpti_EventID *) malloc(eventIdsSize);
			CUPTI_CALL(
					cuptiEventGroupGetAttribute(group,
							CUPTI_EVENT_GROUP_ATTR_EVENTS, &eventIdsSize,
							eventIds));
			...
			for (int j = 0; j < numEvents; j++) {
				CUPTI_CALL(
						cuptiEventGroupReadEvent(group,
								CUPTI_EVENT_READ_FLAG_NONE, eventIds[j],
								&valuesSize, values));
				...
				// print collected value
				{
					...
					CUPTI_CALL(
							cuptiEventGetAttribute(eventIds[j],
									CUPTI_EVENT_ATTR_NAME, &eventNameSize,
									eventName));
					eventName[127] = '\0';
					...
					if (numInstances > 1) {
						for (int k = 0; k < numInstances; k++) {
							if (k != 0)
								_DBG(", "); _DBG("%llu", (unsigned long long)values[k]);
						}
					}
...
void print_metric(CUpti_MetricID& id, CUpti_MetricValue& value, stream_t& s) {
	...
	CUPTI_CALL(
			cuptiMetricGetAttribute(id, CUPTI_METRIC_ATTR_VALUE_KIND,
...
struct profiler {
	...
	profiler(const strvec_t& events, const strvec_t& metrics,
			const int device_num = 0) :
			m_event_names(events), m_metric_names(metrics), m_device_num(
					device_num), m_num_metrics(metrics.size()), m_num_events(
					events.size()), m_metric_passes(0), m_event_passes(0) {
		...
		CUPTI_CALL(
				cuptiSubscribe(&m_subscriber,
						(CUpti_CallbackFunc )detail::get_value_callback,
						&m_kernel_data));
		CUPTI_CALL(
				cuptiEnableCallback(1, m_subscriber,
		...
		CUpti_MetricID *metric_ids = (CUpti_MetricID*) calloc(
				sizeof(CUpti_MetricID), m_num_metrics);
		for (int i = 0; i < m_num_metrics; ++i) {
			CUPTI_CALL(
					cuptiMetricGetIdFromName(m_device,
							m_metric_names[i].c_str(), &metric_ids[i]));
		}

		CUpti_EventID *event_ids = (CUpti_EventID*) calloc(
				sizeof(CUpti_EventID), m_num_events);
		for (int i = 0; i < m_num_events; ++i) {
			CUPTI_CALL(
					cuptiEventGetIdFromName(m_device, m_event_names[i].c_str(),
							&event_ids[i]));
		}
		...
		if (m_num_metrics > 0) {
			...
			CUPTI_CALL(
					cuptiMetricCreateEventGroupSets(m_context,
		...
		if (m_num_events > 0) {
			CUPTI_CALL(
					cuptiEventGroupSetsCreate(m_context,
		...
		_LOG("# Metric Passes: %d\n", m_metric_passes); _LOG("# Event Passes: %d\n", m_event_passes);
		...
		for (int i = 0; i < m_metric_passes; ++i) {
			...
			for (int j = 0; j < m_metric_pass_data->sets[i].numEventGroups;
					++j) {
				CUPTI_CALL(
						cuptiEventGroupGetAttribute(
								m_metric_pass_data->sets[i].eventGroups[j],
								CUPTI_EVENT_GROUP_ATTR_NUM_EVENTS,
								&num_events_size, &num_events)); _LOG("  Event Group %d, #Events = %d", j, num_events);
		...
		for (int i = 0; i < m_event_passes; ++i) {
			...
			uint32_t num_events = 0;
			size_t num_events_size = sizeof(num_events);
			for (int j = 0; j < m_event_pass_data->sets[i].numEventGroups;
					++j) {
				CUPTI_CALL(
						cuptiEventGroupGetAttribute(
	...
	void stop() {
		for (auto &k : m_kernel_data) {
			...
			for (int i = 0; i < m_metric_passes; ++i) {
				...
				std::copy(data[i].event_values.begin(),
						data[i].event_values.end(), event_values + running_sum);
				running_sum += data[i].num_events;
			}

			for (int i = 0; i < m_num_metrics; ++i) {
				CUptiResult _status = cuptiMetricGetValue(m_device,
						m_metric_ids[i], total_events * sizeof(CUpti_EventID),
						event_ids, total_events * sizeof(uint64_t),
						event_values, 0, &metric_value);
				if (_status != CUPTI_SUCCESS) {
					fprintf(stderr,
			...
			for (int i = m_metric_passes;
					i < (m_metric_passes + m_event_passes); ++i) {
		...
		CUPTI_CALL(
				cuptiEnableCallback(0, m_subscriber,
						CUPTI_CB_DOMAIN_RUNTIME_API,
						CUPTI_RUNTIME_TRACE_CBID_cudaLaunch_v3020));
		CUPTI_CALL(
				cuptiEnableCallback(0, m_subscriber,
...
static std::vector<std::string> available_metrics(CUdevice device) {
	...
	for (int i = 0; i < numMetric; i++) {
		...
		CUPTI_CALL(
				cuptiMetricGetAttribute(metricIdArray[i],
...
static std::vector<std::string> available_events(CUdevice device) {
	...
	for (int i = 0; i < totalEvents; i++) {
		size = __CUPTI_PROFILER_NAME_SHORT;
		CUPTI_CALL(
				cuptiEventGetAttribute(eventIdArray[i], CUPTI_EVENT_ATTR_NAME,
						&size, eventName));
