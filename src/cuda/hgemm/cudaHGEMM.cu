...
//================== Read test parameters
    ...
    else
    {
        ...
        snprintf(a_matrix_path, 100, "hgemm_a_%i.matrix", (signed int)DEFAULT_INPUT_SIZE);
    ...
    else
    {
        ...
        snprintf(b_matrix_path, 100, "hgemm_b_%i.matrix", (signed int)DEFAULT_INPUT_SIZE);
        printf("Using default input_a path: %s\n", b_matrix_path);
    }
...
//====================================
	...
	{//================== Global test loop
        ...
        if (loop2 || !device_warmup) {
            ...
            if (badass_memcmp((byte*)GOLD, (byte*)C, sizec * sizeof( half ))) {
    			...
    			for(i=0; (i<k); i++)
    			{
    				for(j=0; (j<k); j++)
    				{
						...
						if ((fabs((double)(valOutput-valGold)/valGold) > pow(1.0, -5.0))||(fabs((double)(valOutput-valGold)/valGold) > pow(1.0, -5.0))) {
							#pragma omp critical
							{
								snprintf(error_detail, 150, "p: [%d, %d], r: %1.16e, e: %1.16e", i, j, valOutput, valGold);
								//printf("%s\n", error_detail);
								#ifdef LOGS
