...
{
    copy_ongpu(l.outputs*l.batch, state.input, 1, l.output_gpu, 1, state.st_handle.stream);
    shortcut_gpu(l.batch, l.w, l.h, l.c, state.net.layers[l.index].output_gpu, l.out_w, l.out_h, l.out_c, l.output_gpu, state.st_handle.stream);
...
void backward_shortcut_layer_gpu(const layer l, network_state state)
{
    ...
    axpy_ongpu(l.outputs*l.batch, 1, l.delta_gpu, 1, state.delta, 1, state.st_handle.stream);
    shortcut_gpu(l.batch, l.out_w, l.out_h, l.out_c, l.delta_gpu, l.w, l.h, l.c, state.net.layers[l.index].delta_gpu, state.st_handle.stream);
}
#endif
