...
deconvolutional_layer make_deconvolutional_layer(int batch, int h, int w, int c,
	...
	fprintf(stderr,
			"Deconvolutional Layer: %d x %d x %d image, %d filters -> %d x %d x %d image\n",
...
void forward_deconvolutional_layer(const deconvolutional_layer l,
		network_state state) {
	int i;
	int out_h = deconvolutional_out_height(l);
	int out_w = deconvolutional_out_width(l);
...
void backward_deconvolutional_layer(deconvolutional_layer l,
		network_state state) {
...
void update_deconvolutional_layer(deconvolutional_layer l, float learning_rate,
