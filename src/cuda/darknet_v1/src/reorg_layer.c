		...
		for (k = 0; k < l.c; ++k) {
			for (j = 0; j < l.h; ++j) {
				for (i = 0; i < l.w; ++i) {
					...
					int out_index = w2
							+ l.out_w * (h2 + l.out_h * (c2 + l.out_c * b));
...
{
	reorg_ongpu(state.input, l.w, l.h, l.c, l.batch, l.stride, 1, l.output_gpu, state.st_handle.stream);
}

void backward_reorg_layer_gpu(layer l, network_state state)
{
	reorg_ongpu(l.delta_gpu, l.w, l.h, l.c, l.batch, l.stride, 0, state.delta, state.st_handle.stream);
}
#endif
