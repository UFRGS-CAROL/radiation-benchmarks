...
void optimize_picture(network *net, image orig, int max_layer, float scale,
	...
	copy_ongpu(last.outputs, last.output_gpu, 1, last.delta_gpu, 1, state.st_handle.stream);
...
void reconstruct_picture(network net, float *features, image recon,
		image update, float rate, float momentum, float lambda, int smooth_size,
		int iters) {
	int iter = 0;
...
void run_nightmare(int argc, char **argv) {
	...
	if (reconstruct) {
		...
		image crop = crop_image(out_im, zz, zz, out_im.w - 2 * zz,
				out_im.h - 2 * zz);
		//flip_image(crop);
