...
def main(board):
    ...
    generate = ["mkdir -p /var/radiation-benchmarks/data", "cd " + src_darknet, "make clean GPU=1", "make -j 4 GPU=1 ", "mv ./darknet_v1 " + bin_path + "/"]
    execute = []
    ...
    generate.append("mv ./darknet_v1 " + bin_path + "/")
    ...
    fp = open(installDir + "scripts/how_to_run_darknet_v1_cuda_" + board, 'w')

    for i in execute:
        print >> fp, "[\"""sudo " + str(i) + "\" , 0.016, \"darknet_v1\"],"

    print "\nConfiguring done, to run check file: " + installDir + "scripts/how_to_run_darknet_v1_cuda_" + board + "\n"
