...
template<typename real_t>
int check_output(std::vector<real_t>& gold, std::vector<real_t>& found, int k) {
	int host_errors = 0;

#pragma omp parallel for shared(host_errors)
	for (int i = 0; i < k; i++) {
		for (int j = 0; j < k; j++) {
			int global_index = i * k + j;
			register double valGold = gold[global_index];
			register double valOutput = found[global_index];
...
int main(int argc, char **argv) {
	...
	for (auto it = 0; it < args.iterations; it++) {
		...
		auto errors = 0;
		if (args.generate == false) {
			errors = check_output(gold, c_host, args.k);
