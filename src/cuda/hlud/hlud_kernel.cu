...
__global__ void lud_perimeter(half *m, int matrix_dim, int offset) {
	__shared__ half dia[BLOCK_SIZE][BLOCK_SIZE];
	__shared__ half peri_row[BLOCK_SIZE][BLOCK_SIZE];
	__shared__ half peri_col[BLOCK_SIZE][BLOCK_SIZE];
...
__global__ void lud_internal(half *m, int matrix_dim, int offset) {
	__shared__ half peri_row[BLOCK_SIZE][BLOCK_SIZE];
	__shared__ half peri_col[BLOCK_SIZE][BLOCK_SIZE];
...
void lud_cuda(half *m, int matrix_dim) {
	int i = 0;
	dim3 dimBlock(BLOCK_SIZE, BLOCK_SIZE);
	half *m_debug = (half*) malloc(
			matrix_dim * matrix_dim * sizeof(half));
