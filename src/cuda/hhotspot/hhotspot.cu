...
typedef struct parameters_t {
    ...
    half_float::half *FilesavingTemp, *FilesavingPower, *MatrixOut, *GoldMatrix;
...
half *power,   //power input
half *temp_src,    //temperature input/output
half *temp_dst,    //temperature input/output
...
half time_elapsed)
{
    ...
    __shared__ half power_on_cuda[BLOCK_SIZE][BLOCK_SIZE];
    __shared__ half temp_t[BLOCK_SIZE][BLOCK_SIZE]; // saving temparary temperature result

    half amb_temp = __float2half(80.0);
    half step_div_Cap;
    half Rx_1,Ry_1,Rz_1;
...
int compute_tran_temp(half_float::half *MatrixPower, half_float::half *MatrixTemp[2], int col, int row, \
...
void getParams(int argc, char** argv, parameters *params)
{
    ...
    else
    {
        ...
        snprintf(params -> ofile, 100, "gold_half_%i_%i", params -> grid_rows, params -> sim_time);
...
void run(int argc, char** argv)
{
    ...
    setupParams -> FilesavingTemp = (half_float::half *) malloc(size *sizeof(half_float::half));
    setupParams -> FilesavingPower = (half_float::half *) malloc(size *sizeof(half_float::half));
    setupParams -> MatrixOut = (half_float::half *) calloc (size, sizeof(half_float::half));
    setupParams -> GoldMatrix = (half_float::half *) calloc (size, sizeof(half_float::half));
    ...
    for ( int loop1=0 ; loop1<(setupParams->setup_loops) ; loop1++)
    {
        ...
        half_float::half *MatrixTemp[setupParams->nstreams][2], *MatrixPower[setupParams->nstreams];
