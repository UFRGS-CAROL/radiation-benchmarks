...
__global__ void kernel_gpu_cuda(par_str<real_t> d_par_gpu, dim_str d_dim_gpu,
	...
	if (bx < d_dim_gpu.number_boxes) {
		...
		// parameters
		half_t a2 = half_t(2.0) * d_par_gpu.alpha * d_par_gpu.alpha;

		// home box
		int first_i;
		FOUR_VECTOR<real_t> *rA;
		FOUR_VECTOR<half_t> *fA;
		__shared__ FOUR_VECTOR<half_t> rA_shared[200];
		...
		real_t *qB;
		int j = 0;
		__shared__ FOUR_VECTOR<half_t> rB_shared[200];
		__shared__ half_t qB_shared[200];
		...
		for (k = 0; k < (1 + d_box_gpu[bx].nn); k++) {
			...
			while (wtx < NUMBER_PAR_PER_BOX) {
				...
				for (j = 0; j < NUMBER_PAR_PER_BOX; j++) {
					...
					fA[wtx].v += (half_t)(qB_shared[j] * vij);
					fA[wtx].x += (half_t)(qB_shared[j] * fxij);
					fA[wtx].y += (half_t)(qB_shared[j] * fyij);
					fA[wtx].z += (half_t)(qB_shared[j] * fzij);
				}
				// increment work thread index
				wtx = wtx + NUMBER_THREADS;
			}
...
__global__ void compare_two_outputs(half_t* lhs, real_t* rhs) {
	...
	check_bit_error(lhs[tid], rhs[tid], 0);
}
