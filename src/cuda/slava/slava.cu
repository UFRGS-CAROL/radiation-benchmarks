...
__global__ void kernel_gpu_cuda(par_str d_par_gpu, dim_str d_dim_gpu, box_str* d_box_gpu, FOUR_VECTOR* d_rv_gpu, float* d_qv_gpu, FOUR_VECTOR* d_fv_gpu) {
...
void generateInput(dim_str dim_cpu, char *input_distances, FOUR_VECTOR **rv_cpu, char *input_charges, float **qv_cpu)
{
...
void readInput(dim_str dim_cpu, char *input_distances, FOUR_VECTOR **rv_cpu, char *input_charges, float **qv_cpu, int fault_injection)
{
	FILE *fp;
	int i;
...
int main(int argc, char *argv []) {
	...
	char test_info[200];
	snprintf(test_info, 200, "type:single-precision streams:%d boxes:%d block_size:%d", nstreams, dim_cpu.boxes1d_arg, NUMBER_THREADS);
	printf("%s\n", test_info);
	#ifdef LOGS
