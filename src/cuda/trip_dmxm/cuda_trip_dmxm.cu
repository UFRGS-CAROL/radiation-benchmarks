...
//====================================
	...
	checkFrameworkErrors( cudaMemcpy( d_A[2], A, matrixSize * sizeof( double ), cudaMemcpyHostToDevice ) ); // PUSH A

	checkFrameworkErrors( cudaMemcpy( d_B[0], B, matrixSize * sizeof( double ), cudaMemcpyHostToDevice ) ); // PUSH B
	checkFrameworkErrors( cudaMemcpy( d_B[1], B, matrixSize * sizeof( double ), cudaMemcpyHostToDevice ) ); // PUSH B
	checkFrameworkErrors( cudaMemcpy( d_B[2], B, matrixSize * sizeof( double ), cudaMemcpyHostToDevice ) ); // PUSH B
}
...
//====================================
	...
	for(loop2=0; loop2<iterations; loop2++)
	{
        ...
        if (loop2 || !device_warmup) {
            checkFrameworkErrors( cudaMemcpy(C[0], d_C[0], matrixSize * sizeof( double ), cudaMemcpyDeviceToHost) );
            checkFrameworkErrors( cudaMemcpy(C[1], d_C[1], matrixSize * sizeof( double ), cudaMemcpyDeviceToHost) );
            checkFrameworkErrors( cudaMemcpy(C[2], d_C[2], matrixSize * sizeof( double ), cudaMemcpyDeviceToHost) );
			checkOutputErrors();
        }
		//====================================
