...
// printf("Write\n");
	...
	for (int i = 0; i < DEFAULT_INPUT_SIZE; i++) {
		fwrite(&(h_A[i * DEFAULT_INPUT_SIZE]),
				sizeof(half_float::half) * DEFAULT_INPUT_SIZE, 1, f_A);
	}
	...
	for (int i = 0; i < DEFAULT_INPUT_SIZE; i++) {
		fwrite(&(h_B[i * DEFAULT_INPUT_SIZE]),
				sizeof(half_float::half) * DEFAULT_INPUT_SIZE, 1, f_B);
	}
	printf("Done\n");
...
half_float::half* openmpMul(half_float::half* a, half_float::half* b,
		size_t size) {
	double time = mysecond();

	half_float::half* bT = (half_float::half*) malloc(
			sizeof(half_float::half) * size * size);
	half_float::half* c = (half_float::half*) calloc(size * size,
...
void generateGoldMatrixHalf(unsigned char use_tensor_cores) {
	...
	checkCudaErrors(
			cudaMemset(d_C, half_float::half(0.0), sizec * sizeof(half))); // ZERA C

	checkCudaErrors(
			cudaMemcpy(d_A, A, sizea * sizeof(half), cudaMemcpyHostToDevice)); // PUSH A

	checkCudaErrors(
			cudaMemcpy(d_B, B, sizeb * sizeof(half), cudaMemcpyHostToDevice)); // PUSH B
	...
	checkCudaErrors(
			cudaMemcpy(GOLD, d_C, sizec * sizeof(half),
		...
		for (i = 0; i < k; i++) {
			for (j = 0; j < k; j++) {
				register float diff = fabs(
						((float) hostGold[i * k + j] - (float) GOLD[i * k + j])
								/ (float) hostGold[i * k + j]);
				if (diff > maxDiff) {
#pragma omp critical
		...
		printf(
				"CPU and GPU match by an error of up to %e element difference. Writing to file...\n",
				maxDiff);
	}
...
//====================================
	...
	printf("Each input matrix size: %.4fGB\n",
			(float) sizeof(half_float::half) * DEFAULT_INPUT_SIZE
					* DEFAULT_INPUT_SIZE / (1024 * 1024 * 1024));
