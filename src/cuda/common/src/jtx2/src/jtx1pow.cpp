 ...
 */
static void jtx1_get_ina3221_sysf(jtx2_rail rail, jtx2_rail_type measure,
		unsigned int *val) {
	FILE *fp;
	int addr;
	int ans;
...
//	char *mea = "voltage";
	...
	if (rail >= 0 && rail <= 2) {
		...
		addr = 2;
	} else if (rail >= 9 && rail <= 11) {
		addr = 3;
	} else {
		printf("Invalid Rail Option - Setting to default Value 2\n");
		addr = 2;
	}
...
static int jtx1_get_ina3221_userspace_i2c(int rail, unsigned int *val) {
	printf("Nunca deveria entrar aqui\n");
	int file;
	int adapter_nr = 1;
	char filename[20];
 ...
 * @param *val Output's reference
 */
void jtx1_get_ina3221(jtx2_rail rail, jtx2_rail_type measure,
		unsigned int *val) {
	...
	if (rail >= 0 && rail <= 11) {
		...
		printf(" Rail parameter out of range\n");
	}
	// } else {}
