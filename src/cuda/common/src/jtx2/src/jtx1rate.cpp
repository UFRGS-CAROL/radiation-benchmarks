 ...
 */
static void jtx1_set_cpu_rate(const unsigned long cpu_rate,
...
void jtx1_get_rate(const jtx1_unit unit, unsigned long *rate) {
	...
	switch (unit) {
	case EMC_RATE: {
		fp = fopen("/sys/kernel/debug/clk/emc/clk_rate", "r");
		break;
	}
	case GPU_RATE: {
		fp = fopen("/sys/kernel/debug/clk/gbus/clk_rate", "r");
		break;
	}
	case CPU0_RATE: {
		fp = fopen("/sys/devices/system/cpu/cpu0/cpufreq/scaling_cur_freq",
				"r");
		break;
	}
	case CPU1_RATE: {
		fp = fopen("/sys/devices/system/cpu/cpu1/cpufreq/scaling_cur_freq",
				"r");
		break;
	}
	case CPU2_RATE: {
		fp = fopen("/sys/devices/system/cpu/cpu2/cpufreq/scaling_cur_freq",
				"r");
		break;
	}
	case CPU3_RATE: {
		fp = fopen("/sys/devices/system/cpu/cpu3/cpufreq/scaling_cur_freq",
				"r");
		break;
	}
	default:
