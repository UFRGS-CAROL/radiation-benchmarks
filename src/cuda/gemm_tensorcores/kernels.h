...
__global__ void compute_gemm(const half_t *A0, const half_t *A1, const half_t *A2, const half_t *B0, const half_t *B1, const half_t *B2, const real_t *C0, const real_t *C1, const real_t *C2, real_t *D0, real_t *D1, real_t *D2, float alpha, float beta)
{
...
//        wmma::fragment<wmma::accumulator, M, N, K, float> c0[WARP_COL_TILES][WARP_ROW_TILES];
//        wmma::fragment<wmma::accumulator, M, N, K, float> c1[WARP_COL_TILES][WARP_ROW_TILES];
//        wmma::fragment<wmma::accumulator, M, N, K, float> c2[WARP_COL_TILES][WARP_ROW_TILES];
...
//
//                wmma::load_matrix_sync(c0[i][j], tile_ptr, SHMEM_STRIDE, C_LAYOUT);
//                wmma::load_matrix_sync(c1[i][j], tile_ptr, SHMEM_STRIDE, C_LAYOUT);
//                wmma::load_matrix_sync(c2[i][j], tile_ptr, SHMEM_STRIDE, C_LAYOUT);
//            }
//        }
//
//        __syncthreads();
...
//                lane_ptr = (int4*)((half_t*)lane_ptr + K_GLOBAL * CHUNK_COPY_LINES_PER_WARP);
...
//                wmma::fragment<wmma::matrix_a, M, N, K, half, wmma::row_major> a0[WARP_COL_TILES];
//                wmma::fragment<wmma::matrix_a, M, N, K, half, wmma::row_major> a1[WARP_COL_TILES];
//                wmma::fragment<wmma::matrix_a, M, N, K, half, wmma::row_major> a2[WARP_COL_TILES];
//                
//                wmma::fragment<wmma::matrix_b, M, N, K, half, wmma::col_major> b0[WARP_ROW_TILES];
//                wmma::fragment<wmma::matrix_b, M, N, K, half, wmma::col_major> b1[WARP_ROW_TILES];
//                wmma::fragment<wmma::matrix_b, M, N, K, half, wmma::col_major> b2[WARP_ROW_TILES];
//#pragma unroll
//                for (int i = 0; i < WARP_COL_TILES; i++) {
//                    size_t shmem_idx_a = (warpId/2) * M * 2 + (i * M);
//                    const half_t *tile_ptr = &shmem[shmem_idx_a][k_step * K];
//
//                    wmma::load_matrix_sync(a0[i], tile_ptr, K * CHUNK_K + SKEW_HALF);
//                    wmma::load_matrix_sync(a1[i], tile_ptr, K * CHUNK_K + SKEW_HALF);
//                    wmma::load_matrix_sync(a2[i], tile_ptr, K * CHUNK_K + SKEW_HALF);
//
//#pragma unroll
//                    for (int j = 0; j < WARP_ROW_TILES; j++) {
//                        if (i == 0) {
...
//
//                            wmma::load_matrix_sync(b0[j], tile_ptr, K * CHUNK_K + SKEW_HALF);
//                            wmma::load_matrix_sync(b1[j], tile_ptr, K * CHUNK_K + SKEW_HALF);
//                            wmma::load_matrix_sync(b2[j], tile_ptr, K * CHUNK_K + SKEW_HALF);
//                        }
//
//                        wmma::mma_sync(c0[i][j], a0[i], b0[j], c0[i][j]);
//                        wmma::mma_sync(c1[i][j], a1[i], b1[j], c1[i][j]);
...
//
//                wmma::store_matrix_sync(tile_ptr, c0[i][j], SHMEM_STRIDE, C_LAYOUT);
//                wmma::store_matrix_sync(tile_ptr, c1[i][j], SHMEM_STRIDE, C_LAYOUT);
//                wmma::store_matrix_sync(tile_ptr, c2[i][j], SHMEM_STRIDE, C_LAYOUT);
//            }
//        }
//
//        __syncthreads();
...
//        real_t *dst_gmem_warp_stream_ptr = &D0[gmem_idx];
//        real_t *dst_gmem_warp_stream_ptr = &D1[gmem_idx];
//        real_t *dst_gmem_warp_stream_ptr = &D2[gmem_idx];
//
//#pragma unroll
//        for (int i = 0; i < K; i++) {
...
//__device__ wmma::fragment<wmma::matrix_a, WMMA_M, WMMA_N, WMMA_K, half,
//	wmma::row_major> inline read_voter_wmma(wmma::fragment<wmma::matrix_a, WMMA_M, WMMA_N, WMMA_K, half,
//		wmma::row_major> a_frag) {
//	
//	printf("entrou frag \n");
//	return a_frag;
...
__device__ wmma::fragment<wmma::matrix_a, WMMA_M, WMMA_N, WMMA_K, half_t,
	wmma::row_major> inline read_voter_wmma_for_a(wmma::fragment<wmma::matrix_a, WMMA_M, WMMA_N, WMMA_K, half_t,
		wmma::row_major> &v1,wmma::fragment<wmma::matrix_a, WMMA_M, WMMA_N, WMMA_K, half_t,
	...
	for(int i = 0; i < v1.num_elements ; i ++){
		...
		if (v1.x[i] != v2.x[i] && v2.x[i] != v3.x[i] && v1.x[i] != v3.x[i]) {
			atomicAdd(is_memory_bad, 1);
		}		
	}
	return v1;
...
template<class half_t>
__device__ wmma::fragment<wmma::matrix_b, WMMA_M, WMMA_N, WMMA_K, half_t,
			wmma::col_major> inline read_voter_wmma_for_b(wmma::fragment<wmma::matrix_b, WMMA_M, WMMA_N, WMMA_K, half,wmma::col_major> &v1,
...
//__device__ half inline read_voter_wmma_for(half* v1, half* v2, half* v3, int num_elements,  unsigned long long int* is_memory_bad) {
//	
//	
//	
