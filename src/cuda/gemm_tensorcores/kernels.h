...
// template<class half_t, class real_t, class fragment>
...
__device__ float inline error_voter (float d_shared[][threadIdx.y], wmma::fragment<wmma::accumulator, WMMA_M, WMMA_N, WMMA_K, float,
wmma::row_major> &acc_frag){
	
	register float error_checker = d_shared - acc_frag;
	if (error_checker > 0) {
		atomicAdd(&errors, 1);
		return errors;
	}
