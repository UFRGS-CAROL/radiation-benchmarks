...
__device__ float inline error_voter (wmma::fragment<wmma::accumulator, WMMA_M, WMMA_N, WMMA_K, float, wmma::row_major> &acc_frag){
	
	register float error_checker = acc_frag;
	if (error_checker > 0) {
		atomicAdd(&errors, 1);
		return errors;
	}
