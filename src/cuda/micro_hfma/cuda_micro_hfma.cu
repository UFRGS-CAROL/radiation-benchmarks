...
#define INPUT_A 1.0005
#define INPUT_B 1.0005
...
typedef half_float::half h_tested_type;
...
__global__ void MicroBenchmarkKernel (tested_type *d_R0, tested_type *d_R1, tested_type *d_R2)
{
	...
	for (register unsigned int count = 0; count < OPS; count++) {
		acc = __hfma2(__float2half2_rn(INPUT_A), __float2half2_rn(INPUT_B), acc);
	}
	
	*((half2*)&(d_R0[blockIdx.x * blockDim.x + threadIdx.x])) = acc;
	*((half2*)&(d_R1[blockIdx.x * blockDim.x + threadIdx.x])) = acc;
	*((half2*)&(d_R2[blockIdx.x * blockDim.x + threadIdx.x])) = acc;
}
...
void checkOutputErrors() {
	...
	for(int i=0; i<r_size; i++)
	{
		...
		register h_tested_type valOutput0 = (h_tested_type)R[0][i];
		register h_tested_type valOutput1 = (h_tested_type)R[1][i];
		register h_tested_type valOutput2 = (h_tested_type)R[2][i];
		register h_tested_type valOutput = valOutput0;
...
#ifdef LOGS
	char test_info[90];
	snprintf(test_info, 90, "ops:%d type:half-precision-triplicated", OPS);
	start_log_file((char *)"cuda_micro_sfma", test_info);
#endif
//====================================

//================== Alloc HOST memory
	R[0] = ( h_tested_type* ) malloc( r_size * sizeof( tested_type ) );
	R[1] = ( h_tested_type* ) malloc( r_size * sizeof( tested_type ) );
	R[2] = ( h_tested_type* ) malloc( r_size * sizeof( tested_type ) );
...
//====================================
	...
	for(loop2=0; loop2<iterations; loop2++)
	{
		...
		MicroBenchmarkKernel<<<gridsize, (int)(blocksize / 2.0)>>>(d_R[0], d_R[1], d_R[2]);
