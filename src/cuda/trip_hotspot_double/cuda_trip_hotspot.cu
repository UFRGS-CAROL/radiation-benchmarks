...
typedef struct parameters_t {
	...
	double *FilesavingTemp1, *FilesavingPower1, *MatrixOut1, *GoldMatrix1;
	double *FilesavingTemp2, *FilesavingPower2, *MatrixOut2;
	double *FilesavingTemp3, *FilesavingPower3, *MatrixOut3;
...
void readInput(parameters *params) {
	...
	if (params->fault_injection) {
		...
		printf("!!!!!!!!! Injected error: FilesavingTemp[32] = %lf\n",
...
__device__ double inline read_voter(double *v1, double *v2, double *v3,
		int offset) {

	register double in1 = v1[offset];
	register double in2 = v2[offset];
	register double in3 = v3[offset];
...
__device__ double inline read_voter_2d(double v1[][BLOCK_SIZE],
		double v2[][BLOCK_SIZE], double v3[][BLOCK_SIZE], int x, int y) {
	register double in1 = v1[x][y];
	register double in2 = v2[x][y];
	register double in3 = v3[x][y];
...
__global__ void calculate_temp(int iteration,  //number of iteration
		//Hardening against bad boards
		double* power1,   //power input
		double* temp_src1,    //temperature input/output
		double* temp_dst1,    //temperature input/output
		//---------------------------------------------------
		double* power2,   //power input
		double* temp_src2,    //temperature input/output
		double* temp_dst2,    //temperature input/output
		//---------------------------------------------------
		double* power3,   //power input
		double* temp_src3,    //temperature input/output
		double* temp_dst3,    //temperature input/output
		//---------------------------------------------------
		...
		double Rx, double Ry, double Rz, double step, double time_elapsed) {

	//----------------------------------------------------
	__shared__ double temp_on_cuda_1[BLOCK_SIZE][BLOCK_SIZE];
	__shared__ double power_on_cuda_1[BLOCK_SIZE][BLOCK_SIZE];
	__shared__ double temp_t_1[BLOCK_SIZE][BLOCK_SIZE]; // saving temporary temperature result

	//----------------------------------------------------
	__shared__ double temp_on_cuda_2[BLOCK_SIZE][BLOCK_SIZE];
	__shared__ double power_on_cuda_2[BLOCK_SIZE][BLOCK_SIZE];
	__shared__ double temp_t_2[BLOCK_SIZE][BLOCK_SIZE]; // saving temporary temperature result

	//----------------------------------------------------
	__shared__ double temp_on_cuda_3[BLOCK_SIZE][BLOCK_SIZE];
	__shared__ double power_on_cuda_3[BLOCK_SIZE][BLOCK_SIZE];
	__shared__ double temp_t_3[BLOCK_SIZE][BLOCK_SIZE]; // saving temporary temperature result
	//---------------------------------------------------
	...
	for (int i = 0; i < iteration; i++) {
		...
		IN_RANGE(ty, validYmin, validYmax)) {
			computed = true;
			register double calculated = read_voter_2d(temp_on_cuda_1,
...
int compute_tran_temp(
		//Memory triplication
		double *MatrixPower1, double *MatrixPower2, double *MatrixPower3,
		double *MatrixTemp1[2], double *MatrixTemp2[2], double *MatrixTemp3[2],
		//-------------------------------------------------------------
...
void getParams(int argc, char** argv, parameters *params) {
	...
	if (checkCmdLineFlag(argc, (const char **) argv, "gold_file")) {
		...
		snprintf(params->ofile, 100, "gold_double_%i_%i", params->grid_rows,
				params->sim_time);
...
# define EXPAND_RATE 2// add one iteration will extend the pyramid base by 2 per each borderline
	...
	//HARDENING AGAINST BAD BOARDS
	//-----------------------------------------------------------------------------------
	setupParams->FilesavingTemp1 = (double *) malloc(size * sizeof(double));
	setupParams->FilesavingPower1 = (double *) malloc(size * sizeof(double));
	setupParams->MatrixOut1 = (double *) calloc(size, sizeof(double));
	setupParams->GoldMatrix1 = (double *) calloc(size, sizeof(double));

	setupParams->FilesavingTemp2 = (double *) malloc(size * sizeof(double));
	setupParams->FilesavingPower2 = (double *) malloc(size * sizeof(double));
	setupParams->MatrixOut2 = (double *) calloc(size, sizeof(double));

	setupParams->FilesavingTemp3 = (double *) malloc(size * sizeof(double));
	setupParams->FilesavingPower3 = (double *) malloc(size * sizeof(double));
	setupParams->MatrixOut3 = (double *) calloc(size, sizeof(double));
	...
	if (!(setupParams->generate)) start_log_file("cuda_trip_double_hotspot", test_info);
#endif
	...
	//HARDENING AGAINST BAD BOARDS
	//-----------------------------------------------------------------------------------
	double *MatrixTemp1[setupParams->nstreams][2],
			*MatrixPower1[setupParams->nstreams];

	double *MatrixTemp2[setupParams->nstreams][2],
			*MatrixPower2[setupParams->nstreams];

	double *MatrixTemp3[setupParams->nstreams][2],
	...
	for (int streamIdx = 0; streamIdx < (setupParams->nstreams); streamIdx++) {
		...
		//-----------------------------------------------------------------------------------
		for(int z = 0; z < 2; z++) {
			safe_cuda_malloc_cover((void**)&(MatrixTemp1[streamIdx][z]), sizeof(double)*size);
//		safe_cuda_malloc_cover((void**)&(MatrixTemp1[streamIdx][1]), sizeof(double)*size);

			safe_cuda_malloc_cover((void**)&(MatrixTemp2[streamIdx][z]), sizeof(double)*size);
//		safe_cuda_malloc_cover((void**)&(MatrixTemp2[streamIdx][1]), sizeof(double)*size);

			safe_cuda_malloc_cover((void**)&(MatrixTemp3[streamIdx][z]), sizeof(double)*size);
//		safe_cuda_malloc_cover((void**)&(MatrixTemp3[streamIdx][1]), sizeof(double)*size);
		}
		//-----------------------------------------------------------------------------------
		...
		//HARDENING AGAINST BAD BOARDS
		//-----------------------------------------------------------------------------------
		safe_cuda_malloc_cover((void**)&(MatrixPower1[streamIdx]), sizeof(double)*size);
		safe_cuda_malloc_cover((void**)&(MatrixPower2[streamIdx]), sizeof(double)*size);
		safe_cuda_malloc_cover((void**)&(MatrixPower3[streamIdx]), sizeof(double)*size);
...
//			//-----------------------------------------------------------------------------------
//
//			safe_cuda_malloc_cover((void**)&(MatrixTemp1[streamIdx][0]), sizeof(double)*size);
//			safe_cuda_malloc_cover((void**)&(MatrixTemp1[streamIdx][1]), sizeof(double)*size);
//
//			safe_cuda_malloc_cover((void**)&(MatrixTemp2[streamIdx][0]), sizeof(double)*size);
//			safe_cuda_malloc_cover((void**)&(MatrixTemp2[streamIdx][1]), sizeof(double)*size);
//
//			safe_cuda_malloc_cover((void**)&(MatrixTemp3[streamIdx][0]), sizeof(double)*size);
//			safe_cuda_malloc_cover((void**)&(MatrixTemp3[streamIdx][1]), sizeof(double)*size);
//			//-----------------------------------------------------------------------------------
//
//#else
...
//#endif
			...
			cudaMemset(MatrixTemp2[streamIdx][1], 0.0, sizeof(double) * size);
			...
			cudaMemcpy(MatrixTemp3[streamIdx][0], setupParams->FilesavingTemp3,
					sizeof(double) * size, cudaMemcpyHostToDevice);
			cudaMemset(MatrixTemp3[streamIdx][1], 0.0, sizeof(double) * size);
...
#pragma omp parallel for
		...
		if (setupParams->verbose) {
			...
			printf(
					"[Iteration #%i] SIZE:%d OUTPUT/S:%lf FLOPS: %lf (GFLOPS: %.2f)\n",
...
//					streamIdx++) {
//				memset(setupParams->MatrixOut, 0, sizeof(double) * size);
...
//						register double *ptrGold = &(setupParams->GoldMatrix[i
//								* (setupParams->grid_rows) + 0]);
//						register double *ptrOut = &(setupParams->MatrixOut[i
//								* (setupParams->grid_rows) + 0]);
...
//#pragma omp parallel for shared(host_errors)
	for (int i = 0; i < setup_parameters->grid_rows; i++) {
		for (int j = 0; j < setup_parameters->grid_cols; j++) {
			...
			register double valGold = setup_parameters->GoldMatrix1[index];
			register double valOutput1 = setup_parameters->MatrixOut1[index];
			register double valOutput2 = setup_parameters->MatrixOut2[index];
			register double valOutput3 = setup_parameters->MatrixOut3[index];
			register double valOutput = valOutput1;
